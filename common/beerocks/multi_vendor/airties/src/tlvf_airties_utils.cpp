...
{
    //const std::string bridge_name = "br-lan";
...
bool devicemetrics_get_radio_info(std::shared_ptr<airties::tlvAirtiesDeviceMetrics> &tlvDevMetrics)
{
    ...
    LOG(INFO) << "Number of radios  " << num_of_radios;
...
#else
bool get_radio_info_from_dm(int radio_index, std::string &id_value, uint32_t &radio_temperature)
{
    std::string dm           = "Device.";
    std::string dm_path      = "Device.WiFi.Radio.";
    std::string mac_param    = ".BaseMACAddress";
    std::string temp_param   = ".Temperature";
    std::string stats_string = ".Stats";
    std::string radpath = "", get_path = "";
    static constexpr int amxb_get_timeout = 1;
    uint32_t radio_temp                   = 0;
    const char *radio_id;
    ...
    //Radio ID
    radpath = dm_path + std::to_string(radio_index) + mac_param;
    if (amxb_get(ctx, radpath.c_str(), 0, &data, amxb_get_timeout) != AMXB_STATUS_OK) {
        LOG(ERROR) << "Timed out for amxb_get";
        return false;
    }
    if (amxc_var_is_null(&data)) {
        LOG(ERROR) << "axmc var is null";
        return false;
    }

    get_path = "0.'" + dm_path + std::to_string(radio_index) + ".'" + mac_param;
    radio_id = GETP_CHAR(&data, get_path.c_str());
    //Store the radio id value in the passed arguent.
    id_value = radio_id;

    //Using the same Context, Fetch Radio Temperature
    radpath = dm_path + std::to_string(radio_index) + stats_string + temp_param;
    ...
    if (amxc_var_is_null(&data)) {
        LOG(ERROR) << "axmc is null";
        return false;
    }

    get_path   = "0.'" + dm_path + std::to_string(radio_index) + stats_string + ".'" + temp_param;
    radio_temp = GETP_UINT32(&data, get_path.c_str());
    radio_temperature = radio_temp;
...
bool devicemetrics_get_radio_info(std::shared_ptr<airties::tlvAirtiesDeviceMetrics> &tlvDevMetrics)
{
    ...
    for (int i = 1; i <= num_of_radios; i++) {
        if (!get_radio_info_from_dm(i, id_value, temp_value)) {
            LOG(ERROR) << "Failed to get the Radio information  for  radio: " << i;
            continue;
        }

        m_radio_mac = tlvf::mac_from_string(id_value);

        //Save in the TLV
        auto radio_list        = tlvDevMetrics->create_radio_list();
        radio_list->radio_id() = m_radio_mac;

        radio_list->radio_temperature() = temp_value;
        tlvDevMetrics->add_radio_list(radio_list);
    }
    return true;
}
#endif
