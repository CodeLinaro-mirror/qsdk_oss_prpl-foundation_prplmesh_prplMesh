...
bool ap_wlan_hal_whm::sta_association_control(
    ...
    wfa_map::tlvClientAssociationControlRequest::eAssociationControl association_control)
{
    ...
    case wfa_map::tlvClientAssociationControlRequest::TIMED_BLOCK:
        return sta_deny_temp(mac, bssid);
    ...
    case wfa_map::tlvClientAssociationControlRequest::UNBLOCK: {
        bool ret = sta_allow_temp(mac, bssid);
        ret |= sta_allow(mac, bssid);
        return ret;
    }
    default:
...
bool ap_wlan_hal_whm::sta_deny_temp(const sMacAddr &mac, const sMacAddr &bssid)
{
    ...
    const std::string &ifname         = m_radio_info.available_vaps[vap_id].bss;
    const std::string mac_filter_path = wbapi_utils::search_path_mac_filtering(ifname);
...
bool ap_wlan_hal_whm::clear_blacklist()
{
    ...
    for (const auto &vap_info : m_radio_info.available_vaps) {
        ...
        auto clear_entries = [&](const std::string &entries_path, const char *method) {
            auto entries = m_ambiorix_cl.get_object_multi<AmbiorixVariantMapSmartPtr>(entries_path);
            if (!entries) {
                return;
            }
