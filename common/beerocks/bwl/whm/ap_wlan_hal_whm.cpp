...
bool ap_wlan_hal_whm::process_afc_update_event(const AmbiorixVariant *value)
{
    auto parameters = value->find_child("Updates");
    if (!parameters || parameters->empty()) {
        LOG(DEBUG) << "Received AFC Update event without Updates parameter";
        return false;
    }

    std::string status;
    if (!parameters->read_child(status, "InquiryStatus")) {
        LOG(DEBUG) << "Received AFC Update event without InquiryStatus parameter";
        return false;
    }

    if (status != "UPDATE") {
        LOG(DEBUG) << "AFC Update status other than UPDATE: " << status;
        return false;
    }
