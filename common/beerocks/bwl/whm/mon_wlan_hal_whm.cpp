...
bool mon_wlan_hal_whm::update_vap_stations_stats(const std::string &vap_iface_name,
                                                 ...
                                                 bool is_read_unicast)
{
    ...
    std::unordered_map<sMacAddr, SStaStats> vap_sta_stats;
    ...
    for (auto &station_obj : *stations_list) {
        ...
        SStaStats station_stats;
        read_station_stats(station_obj, station_stats, is_read_unicast);
        auto sta_mac           = tlvf::mac_from_string(mac_addr);
        vap_sta_stats[sta_mac] = station_stats;
    }
    ...
    // Log only station-count changes to validate the bulk RPC path without per-poll noise.
    ...
    // Keep only requested stations and preserve accumulated RSSI/SNR samples.
    std::unordered_map<sMacAddr, SStaStats> updated_sta_stats;
    for (const auto &sta_mac : sta_macs) {
        auto stats_it = vap_sta_stats.find(sta_mac);
...
bool mon_wlan_hal_whm::process_sta_connected_event(
    ...
    const AmbiorixVariant *value, const std::string &sta_path, const std::string &vap_path)
{
    ...
    if (key == "AuthenticationState") {
        ...
        // Disassociation is the primary signal. Active=false is a fallback that
        // prevents a missing notification from leaving a stale monitor entry.
