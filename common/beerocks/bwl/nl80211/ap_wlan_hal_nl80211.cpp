                                        ...
                                        const son::wireless_utils::sBssInfoConf &bss)
{
    ...
    if (bss.authentication_type == WSC::eWscAuth::WSC_AUTH_OPEN) {
        if (bss.encryption_type == WSC::eWscEncr::WSC_ENCR_AES) {
            ...
            ieee80211w.assign("2");
        } else if (bss.encryption_type != WSC::eWscEncr::WSC_ENCR_NONE) {
        ...
        if (bss.encryption_type != WSC::eWscEncr::WSC_ENCR_AES) {
            LOG(ERROR) << "Autoconfiguration:  " << vap_id << " CCMP(AES) is required for SAE-EXT-KEY";
            return false;
        }
        wpa_pairwise.assign("CCMP");
...
void ap_wlan_hal_nl80211::read_akm_cipher_support()
{
    ...
    for (const char *field : k_capability_fields) {
        ...
        if (wpa_ctrl_send_msg(cmd, &reply) && reply) {
...
{
    LOG(TRACE) << __func__ << " vap_id: " << params.vap_id
               << " mac: " << tlvf::mac_to_string(params.mac)
    ...
    cmd += std::string() + " neighbor=" + tlvf::mac_to_string(params.bssid) + ",0," +
           std::to_string(params.oper_class) + "," + std::to_string(params.chan) + ",0";
    ...
    const std::string &bridge_ifname)
{
    ...
    if (son::wireless_utils::try_get_allowed_wifi_generations(bss_info_conf_list, allowed_gens)) {
        for (const auto &entry : son::wireless_utils::hostapd_wifi_generation_flags(allowed_gens)) {
            conf.set_create_head_value(entry.first, entry.second);
        }
    }
