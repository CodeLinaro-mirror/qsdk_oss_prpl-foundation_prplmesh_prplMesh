    ...
    if (band_info_it != radio_info.bands.end()) {
        ...
        for (auto const &pair : band_info_it->supported_channels) {
            ...
            for (auto bw : supported_channel_info.supported_bandwidths) {
                ...
                // This means it will update the rank only on the initial rferesh_radio_info,
...
bool ap_wlan_hal_dwpal::apply_preamble_channel_puncturing(int chan, int vht_center_frequency,
                                               uint16_t disabled_subchannel_bitmap)
{
    // Indicate that channel puncturing is not implemented in this class.
    LOG(ERROR) << "apply_preamble_channel_puncturing is not implemented in ap_wlan_hal_dwpal";
    return false;
}
...
bool ap_wlan_hal_dwpal::process_dwpal_event(char *buffer, int bufLen, const std::string &opcode)
{
    ...
    if (!m_hal_conf.monitored_BSSs.empty()) {
        if (event == Event::STA_Connected || event == Event::STA_Disconnected ||
            ...
            // Need to filter events that are received on unmonitored BSSIDs.
            // Check if the event's BSSID is present in the monitored BSSIDs list.
            // If the event's interface is RADIO, no need to filter
