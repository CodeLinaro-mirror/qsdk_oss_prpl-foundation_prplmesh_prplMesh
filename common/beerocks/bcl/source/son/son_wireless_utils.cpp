...
{
    bool is_320mhz_submode_of_operating_class_band =
        (bw == beerocks::eWiFiBandwidth::BANDWIDTH_320_1 ||
         bw == beerocks::eWiFiBandwidth::BANDWIDTH_320_2) &&
        operating_class_info.band == beerocks::eWiFiBandwidth::BANDWIDTH_320;
    if (operating_class_info.band != bw && !is_320mhz_submode_of_operating_class_band) {
        return false;
    }
    if (operating_class_info.channels.count(channel)) {
        return true;
    }
    ...
    return operating_class_info.channels.count(center_channel) > 0;
}
...
{
    // For now only this function uses these arrays, so define them here.
