...
typedef struct sNodeHostap {
    ...
    //EHT Operations structure (beerocks::net::sEHTOperations) is 10 bytes, radio-level
    uint8_t eht_operations[beerocks::message::EHT_OPERATIONS_SIZE];
...
typedef struct sNodeHostapVendorSpec {
    char iface_name[beerocks::message::IFACE_NAME_LENGTH];
    uint8_t iface_type;
    sMacAddr iface_mac;
    uint8_t ant_num;
...
} __attribute__((packed)) sNodeHostapVendorSpec;
