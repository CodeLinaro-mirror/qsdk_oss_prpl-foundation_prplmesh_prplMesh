...
class cACTION_MONITOR_CLIENT_UNASSOCIATED_STA_LINK_METRIC_REQUEST : public BaseClass
{
    public:
        cACTION_MONITOR_CLIENT_UNASSOCIATED_STA_LINK_METRIC_REQUEST(uint8_t* buff, size_t buff_len, bool parse = false);
        explicit cACTION_MONITOR_CLIENT_UNASSOCIATED_STA_LINK_METRIC_REQUEST(std::shared_ptr<BaseClass> base, bool parse = false);
        ~cACTION_MONITOR_CLIENT_UNASSOCIATED_STA_LINK_METRIC_REQUEST();

        static eActionOp_MONITOR get_action_op(){
            return (eActionOp_MONITOR)(ACTION_MONITOR_CLIENT_UNASSOCIATED_STA_LINK_METRIC_REQUEST);
        }
        const uint16_t& length();
...
class cACTION_MONITOR_CLIENT_UNASSOCIATED_STA_LINK_METRIC_RESPONSE : public BaseClass
{
    public:
        cACTION_MONITOR_CLIENT_UNASSOCIATED_STA_LINK_METRIC_RESPONSE(uint8_t* buff, size_t buff_len, bool parse = false);
        explicit cACTION_MONITOR_CLIENT_UNASSOCIATED_STA_LINK_METRIC_RESPONSE(std::shared_ptr<BaseClass> base, bool parse = false);
        ~cACTION_MONITOR_CLIENT_UNASSOCIATED_STA_LINK_METRIC_RESPONSE();

        static eActionOp_MONITOR get_action_op(){
            return (eActionOp_MONITOR)(ACTION_MONITOR_CLIENT_UNASSOCIATED_STA_LINK_METRIC_RESPONSE);
        }
        const uint16_t& length();
        uint8_t& stations_list_length();
...
class cACTION_MONITOR_CHANNEL_SCAN_ABORT_REQUEST : public BaseClass
{
    public:
        cACTION_MONITOR_CHANNEL_SCAN_ABORT_REQUEST(uint8_t* buff, size_t buff_len, bool parse = false);
        explicit cACTION_MONITOR_CHANNEL_SCAN_ABORT_REQUEST(std::shared_ptr<BaseClass> base, bool parse = false);
        ~cACTION_MONITOR_CHANNEL_SCAN_ABORT_REQUEST();

        static eActionOp_MONITOR get_action_op(){
            return (eActionOp_MONITOR)(ACTION_MONITOR_CHANNEL_SCAN_ABORT_REQUEST);
        }
        void class_swap() override;
        bool finalize() override;
        static size_t get_initial_size();
...
class cACTION_MONITOR_CHANNEL_SCAN_ABORT_RESPONSE : public BaseClass
{
    public:
        cACTION_MONITOR_CHANNEL_SCAN_ABORT_RESPONSE(uint8_t* buff, size_t buff_len, bool parse = false);
        explicit cACTION_MONITOR_CHANNEL_SCAN_ABORT_RESPONSE(std::shared_ptr<BaseClass> base, bool parse = false);
        ~cACTION_MONITOR_CHANNEL_SCAN_ABORT_RESPONSE();

        static eActionOp_MONITOR get_action_op(){
            return (eActionOp_MONITOR)(ACTION_MONITOR_CHANNEL_SCAN_ABORT_RESPONSE);
        }
        uint8_t& success();
        void class_swap() override;
        bool finalize() override;
