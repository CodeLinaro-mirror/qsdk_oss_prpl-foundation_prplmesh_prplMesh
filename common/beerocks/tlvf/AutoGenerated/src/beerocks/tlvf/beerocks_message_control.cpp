...
cACTION_CONTROL_BSS_SET_QOS_MANAGEMENT_REQUEST::cACTION_CONTROL_BSS_SET_QOS_MANAGEMENT_REQUEST(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_CONTROL_BSS_SET_QOS_MANAGEMENT_REQUEST::cACTION_CONTROL_BSS_SET_QOS_MANAGEMENT_REQUEST(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_CONTROL_BSS_SET_QOS_MANAGEMENT_REQUEST::~cACTION_CONTROL_BSS_SET_QOS_MANAGEMENT_REQUEST() {
}
sMacAddr& cACTION_CONTROL_BSS_SET_QOS_MANAGEMENT_REQUEST::bssid() {
    return (sMacAddr&)(*m_bssid);
}

uint8_t& cACTION_CONTROL_BSS_SET_QOS_MANAGEMENT_REQUEST::mscs_enable() {
    return (uint8_t&)(*m_mscs_enable);
}

uint8_t& cACTION_CONTROL_BSS_SET_QOS_MANAGEMENT_REQUEST::scs_enable() {
    return (uint8_t&)(*m_scs_enable);
}

void cACTION_CONTROL_BSS_SET_QOS_MANAGEMENT_REQUEST::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_CONTROL), reinterpret_cast<uint8_t*>(m_action_op));
    m_bssid->struct_swap();
}

bool cACTION_CONTROL_BSS_SET_QOS_MANAGEMENT_REQUEST::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_CONTROL_BSS_SET_QOS_MANAGEMENT_REQUEST::get_initial_size()
{
    size_t class_size = 0;
...
bool cACTION_CONTROL_BSS_SET_QOS_MANAGEMENT_REQUEST::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
