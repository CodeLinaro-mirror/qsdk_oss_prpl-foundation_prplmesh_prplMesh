...
std::shared_ptr<cChannelList> cACTION_APMANAGER_JOINED_NOTIFICATION::create_channel_list() {
    ...
    m_vap_mld_unit_list = (sVapMldUnitsList *)((uint8_t *)(m_vap_mld_unit_list) + len);
...
bool cACTION_APMANAGER_JOINED_NOTIFICATION::add_channel_list(std::shared_ptr<cChannelList> ptr) {
    ...
    m_vap_mld_unit_list = (sVapMldUnitsList *)((uint8_t *)(m_vap_mld_unit_list) + len - ptr->get_initial_size());
...
sVapMldUnitsList& cACTION_APMANAGER_JOINED_NOTIFICATION::vap_mld_unit_list() {
    return (sVapMldUnitsList&)(*m_vap_mld_unit_list);
}
...
void cACTION_APMANAGER_JOINED_NOTIFICATION::class_swap()
{
    ...
    m_vap_mld_unit_list->struct_swap();
}
...
size_t cACTION_APMANAGER_JOINED_NOTIFICATION::get_initial_size()
{
    ...
    class_size += sizeof(sVapMldUnitsList); // vap_mld_unit_list
...
bool cACTION_APMANAGER_JOINED_NOTIFICATION::init()
{
    ...
    m_vap_mld_unit_list = reinterpret_cast<sVapMldUnitsList*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(sVapMldUnitsList))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(sVapMldUnitsList) << ") Failed!";
        return false;
    }
    if (!m_parse__) { m_vap_mld_unit_list->struct_init(); }
...
int8_t& cACTION_APMANAGER_HOSTAP_AP_ENABLED_NOTIFICATION::mld_unit() {
    return (int8_t&)(*m_mld_unit);
}

std::string cACTION_APMANAGER_HOSTAP_AP_ENABLED_NOTIFICATION::configured_ssid_str() {
    char *configured_ssid_ = configured_ssid();
    if (!configured_ssid_) { return std::string(); }
    auto str = std::string(configured_ssid_, m_configured_ssid_idx__);
    auto pos = str.find_first_of('\0');
    if (pos != std::string::npos) {
        str.erase(pos);
    }
...
char* cACTION_APMANAGER_HOSTAP_AP_ENABLED_NOTIFICATION::configured_ssid(size_t length) {
    if( (m_configured_ssid_idx__ == 0) || (m_configured_ssid_idx__ < length) ) {
        TLVF_LOG(ERROR) << "configured_ssid length is smaller than requested length";
        return nullptr;
    }
    return ((char*)m_configured_ssid);
}

bool cACTION_APMANAGER_HOSTAP_AP_ENABLED_NOTIFICATION::set_configured_ssid(const std::string& str) { return set_configured_ssid(str.c_str(), str.size()); }
bool cACTION_APMANAGER_HOSTAP_AP_ENABLED_NOTIFICATION::set_configured_ssid(const char str[], size_t size) {
    if (str == nullptr) {
        TLVF_LOG(WARNING) << "set_configured_ssid received a null pointer.";
        return false;
    }
    ...
    std::copy(str, str + size, m_configured_ssid);
    return true;
}
...
size_t cACTION_APMANAGER_HOSTAP_AP_ENABLED_NOTIFICATION::get_initial_size()
{
    ...
    class_size += beerocks::message::WIFI_SSID_MAX_LENGTH * sizeof(char); // configured_ssid
    return class_size;
}
...
bool cACTION_APMANAGER_HOSTAP_AP_ENABLED_NOTIFICATION::init()
{
    ...
    m_configured_ssid = reinterpret_cast<char*>(m_buff_ptr__);
    ...
    m_configured_ssid_idx__  = beerocks::message::WIFI_SSID_MAX_LENGTH;
    if (m_parse__) { class_swap(); }
    return true;
}
...
sVapMldUnitsList& cACTION_APMANAGER_HOSTAP_VAPS_LIST_UPDATE_NOTIFICATION::vap_mld_unit_list() {
    return (sVapMldUnitsList&)(*m_vap_mld_unit_list);
}
