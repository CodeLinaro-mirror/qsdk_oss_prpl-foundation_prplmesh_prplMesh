...
uint8_t& cACTION_APMANAGER_CONFIGURE::multi_ap_profile() {
...
cACTION_APMANAGER_AFFILIATED_LINK_CHANGED_NOTIFICATION::cACTION_APMANAGER_AFFILIATED_LINK_CHANGED_NOTIFICATION(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_APMANAGER_AFFILIATED_LINK_CHANGED_NOTIFICATION::cACTION_APMANAGER_AFFILIATED_LINK_CHANGED_NOTIFICATION(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_APMANAGER_AFFILIATED_LINK_CHANGED_NOTIFICATION::~cACTION_APMANAGER_AFFILIATED_LINK_CHANGED_NOTIFICATION() {
}
sMacAddr& cACTION_APMANAGER_AFFILIATED_LINK_CHANGED_NOTIFICATION::sta_mld_mac() {
    return (sMacAddr&)(*m_sta_mld_mac);
}

sMacAddr& cACTION_APMANAGER_AFFILIATED_LINK_CHANGED_NOTIFICATION::affiliated_sta_mac() {
    return (sMacAddr&)(*m_affiliated_sta_mac);
}

sMacAddr& cACTION_APMANAGER_AFFILIATED_LINK_CHANGED_NOTIFICATION::bssid() {
    return (sMacAddr&)(*m_bssid);
}

uint8_t& cACTION_APMANAGER_AFFILIATED_LINK_CHANGED_NOTIFICATION::action() {
    return (uint8_t&)(*m_action);
}

void cACTION_APMANAGER_AFFILIATED_LINK_CHANGED_NOTIFICATION::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_APMANAGER), reinterpret_cast<uint8_t*>(m_action_op));
    m_sta_mld_mac->struct_swap();
    m_affiliated_sta_mac->struct_swap();
    m_bssid->struct_swap();
}

bool cACTION_APMANAGER_AFFILIATED_LINK_CHANGED_NOTIFICATION::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_APMANAGER_AFFILIATED_LINK_CHANGED_NOTIFICATION::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(sMacAddr); // sta_mld_mac
    class_size += sizeof(sMacAddr); // affiliated_sta_mac
    class_size += sizeof(sMacAddr); // bssid
    class_size += sizeof(uint8_t); // action
    return class_size;
}

bool cACTION_APMANAGER_AFFILIATED_LINK_CHANGED_NOTIFICATION::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
    ...
    m_affiliated_sta_mac = reinterpret_cast<sMacAddr*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(sMacAddr))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(sMacAddr) << ") Failed!";
        return false;
    }
    if (!m_parse__) { m_affiliated_sta_mac->struct_init(); }
    m_bssid = reinterpret_cast<sMacAddr*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(sMacAddr))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(sMacAddr) << ") Failed!";
        return false;
    ...
    if (!m_parse__) { m_bssid->struct_init(); }
    m_action = reinterpret_cast<uint8_t*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(uint8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint8_t) << ") Failed!";
        return false;
    }
...
cACTION_APMANAGER_RADIO_MODE_CONFIG_REQUEST::cACTION_APMANAGER_RADIO_MODE_CONFIG_REQUEST(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_APMANAGER_RADIO_MODE_CONFIG_REQUEST::cACTION_APMANAGER_RADIO_MODE_CONFIG_REQUEST(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_APMANAGER_RADIO_MODE_CONFIG_REQUEST::~cACTION_APMANAGER_RADIO_MODE_CONFIG_REQUEST() {
}
airties::tlvAirtiesRadioCapability::sStandards& cACTION_APMANAGER_RADIO_MODE_CONFIG_REQUEST::operating_standards() {
    return (airties::tlvAirtiesRadioCapability::sStandards&)(*m_operating_standards);
}

void cACTION_APMANAGER_RADIO_MODE_CONFIG_REQUEST::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_APMANAGER), reinterpret_cast<uint8_t*>(m_action_op));
    m_operating_standards->struct_swap();
}

bool cACTION_APMANAGER_RADIO_MODE_CONFIG_REQUEST::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_APMANAGER_RADIO_MODE_CONFIG_REQUEST::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(airties::tlvAirtiesRadioCapability::sStandards); // operating_standards
    return class_size;
}

bool cACTION_APMANAGER_RADIO_MODE_CONFIG_REQUEST::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
    }
    m_operating_standards = reinterpret_cast<airties::tlvAirtiesRadioCapability::sStandards*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(airties::tlvAirtiesRadioCapability::sStandards))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(airties::tlvAirtiesRadioCapability::sStandards) << ") Failed!";
        return false;
    }
    if (!m_parse__) { m_operating_standards->struct_init(); }
    if (m_parse__) { class_swap(); }
    return true;
}
...
cACTION_APMANAGER_MLD_UPDATE_REQUEST::cACTION_APMANAGER_MLD_UPDATE_REQUEST(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_APMANAGER_MLD_UPDATE_REQUEST::cACTION_APMANAGER_MLD_UPDATE_REQUEST(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_APMANAGER_MLD_UPDATE_REQUEST::~cACTION_APMANAGER_MLD_UPDATE_REQUEST() {
}
std::string cACTION_APMANAGER_MLD_UPDATE_REQUEST::ssid_str() {
    char *ssid_ = ssid();
...
char* cACTION_APMANAGER_MLD_UPDATE_REQUEST::ssid(size_t length) {
...
bool cACTION_APMANAGER_MLD_UPDATE_REQUEST::set_ssid(const std::string& str) { return set_ssid(str.c_str(), str.size()); }
bool cACTION_APMANAGER_MLD_UPDATE_REQUEST::set_ssid(const char str[], size_t size) {
    if (str == nullptr) {
...
int8_t& cACTION_APMANAGER_MLD_UPDATE_REQUEST::mld_unit() {
    return (int8_t&)(*m_mld_unit);
}

uint8_t& cACTION_APMANAGER_MLD_UPDATE_REQUEST::mld_mode() {
...
bool cACTION_APMANAGER_MLD_UPDATE_REQUEST::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_APMANAGER_MLD_UPDATE_REQUEST::get_initial_size()
{
    size_t class_size = 0;
...
bool cACTION_APMANAGER_MLD_UPDATE_REQUEST::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
...
std::string cACTION_APMANAGER_TID_TO_LINK_MAPPING_REQUEST::ssid_str() {
    char *ssid_ = ssid();
    if (!ssid_) { return std::string(); }
    auto str = std::string(ssid_, m_ssid_idx__);
    auto pos = str.find_first_of('\0');
...
char* cACTION_APMANAGER_TID_TO_LINK_MAPPING_REQUEST::ssid(size_t length) {
    if( (m_ssid_idx__ == 0) || (m_ssid_idx__ < length) ) {
        TLVF_LOG(ERROR) << "ssid length is smaller than requested length";
        return nullptr;
    }
...
bool cACTION_APMANAGER_TID_TO_LINK_MAPPING_REQUEST::set_ssid(const std::string& str) { return set_ssid(str.c_str(), str.size()); }
bool cACTION_APMANAGER_TID_TO_LINK_MAPPING_REQUEST::set_ssid(const char str[], size_t size) {
    if (str == nullptr) {
        TLVF_LOG(WARNING) << "set_ssid received a null pointer.";
        return false;
    }
...
uint8_t& cACTION_APMANAGER_TID_TO_LINK_MAPPING_REQUEST::mode() {
    return (uint8_t&)(*m_mode);
}

uint32_t& cACTION_APMANAGER_TID_TO_LINK_MAPPING_REQUEST::advertised_expected_duration() {
    return (uint32_t&)(*m_advertised_expected_duration);
}

uint32_t& cACTION_APMANAGER_TID_TO_LINK_MAPPING_REQUEST::advertised_map_switch_time() {
    return (uint32_t&)(*m_advertised_map_switch_time);
}

std::string cACTION_APMANAGER_TID_TO_LINK_MAPPING_REQUEST::advertised_link_map_frequency_bands_str() {
    char *advertised_link_map_frequency_bands_ = advertised_link_map_frequency_bands();
    if (!advertised_link_map_frequency_bands_) { return std::string(); }
    auto str = std::string(advertised_link_map_frequency_bands_, m_advertised_link_map_frequency_bands_idx__);
    auto pos = str.find_first_of('\0');
    if (pos != std::string::npos) {
        str.erase(pos);
    }
...
char* cACTION_APMANAGER_TID_TO_LINK_MAPPING_REQUEST::advertised_link_map_frequency_bands(size_t length) {
    if( (m_advertised_link_map_frequency_bands_idx__ == 0) || (m_advertised_link_map_frequency_bands_idx__ < length) ) {
        TLVF_LOG(ERROR) << "advertised_link_map_frequency_bands length is smaller than requested length";
        return nullptr;
    }
    return ((char*)m_advertised_link_map_frequency_bands);
}

bool cACTION_APMANAGER_TID_TO_LINK_MAPPING_REQUEST::set_advertised_link_map_frequency_bands(const std::string& str) { return set_advertised_link_map_frequency_bands(str.c_str(), str.size()); }
bool cACTION_APMANAGER_TID_TO_LINK_MAPPING_REQUEST::set_advertised_link_map_frequency_bands(const char str[], size_t size) {
    if (str == nullptr) {
        TLVF_LOG(WARNING) << "set_advertised_link_map_frequency_bands received a null pointer.";
        return false;
    }
    if (size > 32) {
        TLVF_LOG(ERROR) << "Received buffer size is smaller than string length";
    ...
    std::copy(str, str + size, m_advertised_link_map_frequency_bands);
    return true;
}
void cACTION_APMANAGER_TID_TO_LINK_MAPPING_REQUEST::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_APMANAGER), reinterpret_cast<uint8_t*>(m_action_op));
    tlvf_swap(32, reinterpret_cast<uint8_t*>(m_advertised_expected_duration));
    tlvf_swap(32, reinterpret_cast<uint8_t*>(m_advertised_map_switch_time));
}
...
size_t cACTION_APMANAGER_TID_TO_LINK_MAPPING_REQUEST::get_initial_size()
{
    size_t class_size = 0;
    class_size += 32 * sizeof(char); // ssid
    class_size += sizeof(uint8_t); // mode
    class_size += sizeof(uint32_t); // advertised_expected_duration
    class_size += sizeof(uint32_t); // advertised_map_switch_time
    class_size += 32 * sizeof(char); // advertised_link_map_frequency_bands
    return class_size;
}
...
bool cACTION_APMANAGER_TID_TO_LINK_MAPPING_REQUEST::init()
{
    ...
    m_ssid_idx__  = 32;
    m_mode = reinterpret_cast<uint8_t*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(uint8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint8_t) << ") Failed!";
        return false;
    }
    m_advertised_expected_duration = reinterpret_cast<uint32_t*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(uint32_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint32_t) << ") Failed!";
        return false;
    }
    m_advertised_map_switch_time = reinterpret_cast<uint32_t*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(uint32_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint32_t) << ") Failed!";
        return false;
    }
    m_advertised_link_map_frequency_bands = reinterpret_cast<char*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(char) * (32))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(char) * (32) << ") Failed!";
        return false;
    }
    m_advertised_link_map_frequency_bands_idx__  = 32;
    if (m_parse__) { class_swap(); }
    return true;
}
...
cACTION_APMANAGER_QOS_MANAGEMENT_POLICY_REQUEST::cACTION_APMANAGER_QOS_MANAGEMENT_POLICY_REQUEST(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_APMANAGER_QOS_MANAGEMENT_POLICY_REQUEST::cACTION_APMANAGER_QOS_MANAGEMENT_POLICY_REQUEST(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_APMANAGER_QOS_MANAGEMENT_POLICY_REQUEST::~cACTION_APMANAGER_QOS_MANAGEMENT_POLICY_REQUEST() {
}
void cACTION_APMANAGER_QOS_MANAGEMENT_POLICY_REQUEST::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_APMANAGER), reinterpret_cast<uint8_t*>(m_action_op));
}

bool cACTION_APMANAGER_QOS_MANAGEMENT_POLICY_REQUEST::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_APMANAGER_QOS_MANAGEMENT_POLICY_REQUEST::get_initial_size()
{
    size_t class_size = 0;
    return class_size;
}

bool cACTION_APMANAGER_QOS_MANAGEMENT_POLICY_REQUEST::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
...
cACTION_APMANAGER_BSS_SET_QOS_MANAGEMENT_REQUEST::cACTION_APMANAGER_BSS_SET_QOS_MANAGEMENT_REQUEST(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_APMANAGER_BSS_SET_QOS_MANAGEMENT_REQUEST::cACTION_APMANAGER_BSS_SET_QOS_MANAGEMENT_REQUEST(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_APMANAGER_BSS_SET_QOS_MANAGEMENT_REQUEST::~cACTION_APMANAGER_BSS_SET_QOS_MANAGEMENT_REQUEST() {
}
sMacAddr& cACTION_APMANAGER_BSS_SET_QOS_MANAGEMENT_REQUEST::bssid() {
    return (sMacAddr&)(*m_bssid);
}

uint8_t& cACTION_APMANAGER_BSS_SET_QOS_MANAGEMENT_REQUEST::mscs_enable() {
    return (uint8_t&)(*m_mscs_enable);
}

uint8_t& cACTION_APMANAGER_BSS_SET_QOS_MANAGEMENT_REQUEST::scs_enable() {
    return (uint8_t&)(*m_scs_enable);
}

void cACTION_APMANAGER_BSS_SET_QOS_MANAGEMENT_REQUEST::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_APMANAGER), reinterpret_cast<uint8_t*>(m_action_op));
    m_bssid->struct_swap();
}

bool cACTION_APMANAGER_BSS_SET_QOS_MANAGEMENT_REQUEST::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_APMANAGER_BSS_SET_QOS_MANAGEMENT_REQUEST::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(sMacAddr); // bssid
    class_size += sizeof(uint8_t); // mscs_enable
    class_size += sizeof(uint8_t); // scs_enable
    return class_size;
}

bool cACTION_APMANAGER_BSS_SET_QOS_MANAGEMENT_REQUEST::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
    ...
    if (!m_parse__) { m_bssid->struct_init(); }
    m_mscs_enable = reinterpret_cast<uint8_t*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(uint8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint8_t) << ") Failed!";
        return false;
    }
    m_scs_enable = reinterpret_cast<uint8_t*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(uint8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint8_t) << ") Failed!";
        return false;
    }
