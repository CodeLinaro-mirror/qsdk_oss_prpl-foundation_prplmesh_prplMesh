...
sEhtOperationParams& cACTION_BACKHAUL_HOSTAP_CHANNEL_SWITCH_ACS_START::eo_params() {
    return (sEhtOperationParams&)(*m_eo_params);
}

uint8_t& cACTION_BACKHAUL_HOSTAP_CHANNEL_SWITCH_ACS_START::eht_operation_valid() {
...
cACTION_BACKHAUL_PLATFORM_ACS::cACTION_BACKHAUL_PLATFORM_ACS(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_BACKHAUL_PLATFORM_ACS::cACTION_BACKHAUL_PLATFORM_ACS(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_BACKHAUL_PLATFORM_ACS::~cACTION_BACKHAUL_PLATFORM_ACS() {
}
bool cACTION_BACKHAUL_PLATFORM_ACS::isPostInitSucceeded() {
...
std::shared_ptr<airties::cACSChannelList> cACTION_BACKHAUL_PLATFORM_ACS::create_acs_params() {
...
bool cACTION_BACKHAUL_PLATFORM_ACS::add_acs_params(std::shared_ptr<airties::cACSChannelList> ptr) {
    if (ptr == nullptr) {
...
void cACTION_BACKHAUL_PLATFORM_ACS::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_BACKHAUL), reinterpret_cast<uint8_t*>(m_action_op));
    if (m_acs_params_ptr) { m_acs_params_ptr->class_swap(); }
}

bool cACTION_BACKHAUL_PLATFORM_ACS::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_BACKHAUL_PLATFORM_ACS::get_initial_size()
{
    size_t class_size = 0;
    return class_size;
}

bool cACTION_BACKHAUL_PLATFORM_ACS::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
...
cACTION_BACKHAUL_RECONNECT_COMMAND::cACTION_BACKHAUL_RECONNECT_COMMAND(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_BACKHAUL_RECONNECT_COMMAND::cACTION_BACKHAUL_RECONNECT_COMMAND(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_BACKHAUL_RECONNECT_COMMAND::~cACTION_BACKHAUL_RECONNECT_COMMAND() {
}
void cACTION_BACKHAUL_RECONNECT_COMMAND::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_BACKHAUL), reinterpret_cast<uint8_t*>(m_action_op));
}

bool cACTION_BACKHAUL_RECONNECT_COMMAND::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_BACKHAUL_RECONNECT_COMMAND::get_initial_size()
{
    size_t class_size = 0;
    return class_size;
}

bool cACTION_BACKHAUL_RECONNECT_COMMAND::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
...
cACTION_BACKHAUL_WIRED_ONBOARDING_FAILED::cACTION_BACKHAUL_WIRED_ONBOARDING_FAILED(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_BACKHAUL_WIRED_ONBOARDING_FAILED::cACTION_BACKHAUL_WIRED_ONBOARDING_FAILED(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_BACKHAUL_WIRED_ONBOARDING_FAILED::~cACTION_BACKHAUL_WIRED_ONBOARDING_FAILED() {
}
void cACTION_BACKHAUL_WIRED_ONBOARDING_FAILED::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_BACKHAUL), reinterpret_cast<uint8_t*>(m_action_op));
}

bool cACTION_BACKHAUL_WIRED_ONBOARDING_FAILED::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_BACKHAUL_WIRED_ONBOARDING_FAILED::get_initial_size()
{
    size_t class_size = 0;
    return class_size;
}

bool cACTION_BACKHAUL_WIRED_ONBOARDING_FAILED::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
...
cACTION_BACKHAUL_WIRED_CONTROLLER_DETECTED::cACTION_BACKHAUL_WIRED_CONTROLLER_DETECTED(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_BACKHAUL_WIRED_CONTROLLER_DETECTED::cACTION_BACKHAUL_WIRED_CONTROLLER_DETECTED(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_BACKHAUL_WIRED_CONTROLLER_DETECTED::~cACTION_BACKHAUL_WIRED_CONTROLLER_DETECTED() {
}
uint32_t& cACTION_BACKHAUL_WIRED_CONTROLLER_DETECTED::iface_index() {
    return (uint32_t&)(*m_iface_index);
}

void cACTION_BACKHAUL_WIRED_CONTROLLER_DETECTED::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_BACKHAUL), reinterpret_cast<uint8_t*>(m_action_op));
    tlvf_swap(32, reinterpret_cast<uint8_t*>(m_iface_index));
}

bool cACTION_BACKHAUL_WIRED_CONTROLLER_DETECTED::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_BACKHAUL_WIRED_CONTROLLER_DETECTED::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(uint32_t); // iface_index
    return class_size;
}

bool cACTION_BACKHAUL_WIRED_CONTROLLER_DETECTED::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
    }
    m_iface_index = reinterpret_cast<uint32_t*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(uint32_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint32_t) << ") Failed!";
        return false;
    }
...
cACTION_BACKHAUL_HOSTAP_SPATIAL_REUSE_REPORT_NOTIFICATION::cACTION_BACKHAUL_HOSTAP_SPATIAL_REUSE_REPORT_NOTIFICATION(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_BACKHAUL_HOSTAP_SPATIAL_REUSE_REPORT_NOTIFICATION::cACTION_BACKHAUL_HOSTAP_SPATIAL_REUSE_REPORT_NOTIFICATION(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_BACKHAUL_HOSTAP_SPATIAL_REUSE_REPORT_NOTIFICATION::~cACTION_BACKHAUL_HOSTAP_SPATIAL_REUSE_REPORT_NOTIFICATION() {
}
sSpatialReuseParams& cACTION_BACKHAUL_HOSTAP_SPATIAL_REUSE_REPORT_NOTIFICATION::sr_params() {
    return (sSpatialReuseParams&)(*m_sr_params);
}

void cACTION_BACKHAUL_HOSTAP_SPATIAL_REUSE_REPORT_NOTIFICATION::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_BACKHAUL), reinterpret_cast<uint8_t*>(m_action_op));
    m_sr_params->struct_swap();
}

bool cACTION_BACKHAUL_HOSTAP_SPATIAL_REUSE_REPORT_NOTIFICATION::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_BACKHAUL_HOSTAP_SPATIAL_REUSE_REPORT_NOTIFICATION::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(sSpatialReuseParams); // sr_params
    return class_size;
...
bool cACTION_BACKHAUL_HOSTAP_SPATIAL_REUSE_REPORT_NOTIFICATION::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
...
cACTION_BACKHAUL_WIFI_CREDENTIALS_UPDATE_REQUEST::cACTION_BACKHAUL_WIFI_CREDENTIALS_UPDATE_REQUEST(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_BACKHAUL_WIFI_CREDENTIALS_UPDATE_REQUEST::cACTION_BACKHAUL_WIFI_CREDENTIALS_UPDATE_REQUEST(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_BACKHAUL_WIFI_CREDENTIALS_UPDATE_REQUEST::~cACTION_BACKHAUL_WIFI_CREDENTIALS_UPDATE_REQUEST() {
}
sMacAddr& cACTION_BACKHAUL_WIFI_CREDENTIALS_UPDATE_REQUEST::radio_mac() {
    return (sMacAddr&)(*m_radio_mac);
}

bool cACTION_BACKHAUL_WIFI_CREDENTIALS_UPDATE_REQUEST::isPostInitSucceeded() {
    if (!m_wifi_credentials_init) {
        TLVF_LOG(ERROR) << "wifi_credentials is not initialized";
        return false;
    }
    return true; 
}
...
bool cACTION_BACKHAUL_WIFI_CREDENTIALS_UPDATE_REQUEST::add_wifi_credentials(std::shared_ptr<WSC::cEncryptedSettingsPayload> ptr) {
    ...
    m_wifi_credentials_init = true;
    size_t len = ptr->getLen();
    m_wifi_credentials_ptr = ptr;
    if (!buffPtrIncrementSafe(len)) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << len << ") Failed!";
        return false;
    }
...
void cACTION_BACKHAUL_WIFI_CREDENTIALS_UPDATE_REQUEST::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_BACKHAUL), reinterpret_cast<uint8_t*>(m_action_op));
    m_radio_mac->struct_swap();
    if (m_wifi_credentials_ptr) { m_wifi_credentials_ptr->class_swap(); }
}

bool cACTION_BACKHAUL_WIFI_CREDENTIALS_UPDATE_REQUEST::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_BACKHAUL_WIFI_CREDENTIALS_UPDATE_REQUEST::get_initial_size()
{
    size_t class_size = 0;
...
bool cACTION_BACKHAUL_WIFI_CREDENTIALS_UPDATE_REQUEST::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
...
cACTION_BACKHAUL_WIFI_ENABLE_DISABLE_ENDPOINT::cACTION_BACKHAUL_WIFI_ENABLE_DISABLE_ENDPOINT(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_BACKHAUL_WIFI_ENABLE_DISABLE_ENDPOINT::cACTION_BACKHAUL_WIFI_ENABLE_DISABLE_ENDPOINT(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_BACKHAUL_WIFI_ENABLE_DISABLE_ENDPOINT::~cACTION_BACKHAUL_WIFI_ENABLE_DISABLE_ENDPOINT() {
}
sMacAddr& cACTION_BACKHAUL_WIFI_ENABLE_DISABLE_ENDPOINT::radio_mac() {
    return (sMacAddr&)(*m_radio_mac);
}

uint8_t& cACTION_BACKHAUL_WIFI_ENABLE_DISABLE_ENDPOINT::enable() {
    return (uint8_t&)(*m_enable);
}

void cACTION_BACKHAUL_WIFI_ENABLE_DISABLE_ENDPOINT::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_BACKHAUL), reinterpret_cast<uint8_t*>(m_action_op));
    m_radio_mac->struct_swap();
}

bool cACTION_BACKHAUL_WIFI_ENABLE_DISABLE_ENDPOINT::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_BACKHAUL_WIFI_ENABLE_DISABLE_ENDPOINT::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(sMacAddr); // radio_mac
    class_size += sizeof(uint8_t); // enable
...
bool cACTION_BACKHAUL_WIFI_ENABLE_DISABLE_ENDPOINT::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
...
cACTION_BACKHAUL_MLD_UPDATE_REQUEST::cACTION_BACKHAUL_MLD_UPDATE_REQUEST(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_BACKHAUL_MLD_UPDATE_REQUEST::cACTION_BACKHAUL_MLD_UPDATE_REQUEST(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_BACKHAUL_MLD_UPDATE_REQUEST::~cACTION_BACKHAUL_MLD_UPDATE_REQUEST() {
}
sMacAddr& cACTION_BACKHAUL_MLD_UPDATE_REQUEST::radio_mac() {
    return (sMacAddr&)(*m_radio_mac);
}

int8_t& cACTION_BACKHAUL_MLD_UPDATE_REQUEST::mld_unit() {
    return (int8_t&)(*m_mld_unit);
}

uint8_t& cACTION_BACKHAUL_MLD_UPDATE_REQUEST::mld_mode() {
    return (uint8_t&)(*m_mld_mode);
}

void cACTION_BACKHAUL_MLD_UPDATE_REQUEST::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_BACKHAUL), reinterpret_cast<uint8_t*>(m_action_op));
    m_radio_mac->struct_swap();
}

bool cACTION_BACKHAUL_MLD_UPDATE_REQUEST::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_BACKHAUL_MLD_UPDATE_REQUEST::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(sMacAddr); // radio_mac
...
bool cACTION_BACKHAUL_MLD_UPDATE_REQUEST::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
    ...
    if (!m_parse__) { m_radio_mac->struct_init(); }
    m_mld_unit = reinterpret_cast<int8_t*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(int8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(int8_t) << ") Failed!";
        return false;
    }
...
cACTION_BACKHAUL_TOPOLOGY_NOTIFICATION_COMMAND::cACTION_BACKHAUL_TOPOLOGY_NOTIFICATION_COMMAND(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_BACKHAUL_TOPOLOGY_NOTIFICATION_COMMAND::cACTION_BACKHAUL_TOPOLOGY_NOTIFICATION_COMMAND(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_BACKHAUL_TOPOLOGY_NOTIFICATION_COMMAND::~cACTION_BACKHAUL_TOPOLOGY_NOTIFICATION_COMMAND() {
}
void cACTION_BACKHAUL_TOPOLOGY_NOTIFICATION_COMMAND::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_BACKHAUL), reinterpret_cast<uint8_t*>(m_action_op));
}

bool cACTION_BACKHAUL_TOPOLOGY_NOTIFICATION_COMMAND::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_BACKHAUL_TOPOLOGY_NOTIFICATION_COMMAND::get_initial_size()
{
    size_t class_size = 0;
    return class_size;
}

bool cACTION_BACKHAUL_TOPOLOGY_NOTIFICATION_COMMAND::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
...
cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::~cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION() {
}
uint8_t& cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::grant_successful() {
    return (uint8_t&)(*m_grant_successful);
}

uint8_t& cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::regulatory_applicable() {
    return (uint8_t&)(*m_regulatory_applicable);
}

uint16_t& cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::inquiry_request_length() {
    return (uint16_t&)(*m_inquiry_request_length);
}

std::string cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::inquiry_request_str() {
...
char* cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::inquiry_request(size_t length) {
...
bool cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::set_inquiry_request(const std::string& str) { return set_inquiry_request(str.c_str(), str.size()); }
bool cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::set_inquiry_request(const char str[], size_t size) {
    if (str == nullptr) {
...
bool cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::alloc_inquiry_request(size_t count) {
    if (m_lock_order_counter__ > 0) {;
...
uint16_t& cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::inquiry_response_length() {
    return (uint16_t&)(*m_inquiry_response_length);
}

std::string cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::inquiry_response_str() {
...
char* cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::inquiry_response(size_t length) {
...
bool cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::set_inquiry_response(const std::string& str) { return set_inquiry_response(str.c_str(), str.size()); }
bool cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::set_inquiry_response(const char str[], size_t size) {
    if (str == nullptr) {
...
bool cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::alloc_inquiry_response(size_t count) {
...
uint16_t& cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::possible_channels_length() {
    return (uint16_t&)(*m_possible_channels_length);
}

std::string cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::possible_channels_str() {
...
char* cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::possible_channels(size_t length) {
...
bool cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::set_possible_channels(const std::string& str) { return set_possible_channels(str.c_str(), str.size()); }
bool cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::set_possible_channels(const char str[], size_t size) {
    if (str == nullptr) {
...
bool cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::alloc_possible_channels(size_t count) {
...
void cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_BACKHAUL), reinterpret_cast<uint8_t*>(m_action_op));
...
bool cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::get_initial_size()
{
    size_t class_size = 0;
...
bool cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
...
cACTION_BACKHAUL_TRIGGER_ON_BOOT_SCAN::cACTION_BACKHAUL_TRIGGER_ON_BOOT_SCAN(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_BACKHAUL_TRIGGER_ON_BOOT_SCAN::cACTION_BACKHAUL_TRIGGER_ON_BOOT_SCAN(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_BACKHAUL_TRIGGER_ON_BOOT_SCAN::~cACTION_BACKHAUL_TRIGGER_ON_BOOT_SCAN() {
}
sMacAddr& cACTION_BACKHAUL_TRIGGER_ON_BOOT_SCAN::radio_mac() {
    return (sMacAddr&)(*m_radio_mac);
}

void cACTION_BACKHAUL_TRIGGER_ON_BOOT_SCAN::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_BACKHAUL), reinterpret_cast<uint8_t*>(m_action_op));
    m_radio_mac->struct_swap();
}

bool cACTION_BACKHAUL_TRIGGER_ON_BOOT_SCAN::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_BACKHAUL_TRIGGER_ON_BOOT_SCAN::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(sMacAddr); // radio_mac
    return class_size;
...
bool cACTION_BACKHAUL_TRIGGER_ON_BOOT_SCAN::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
