...
main() {
	...
	while true; do
		...
		-o | --openwrt-version)
			...
			shift
			;;
		-u | --openwrt-toolchain-version)
			OPENWRT_TOOLCHAIN_VERSION="$2"
			shift
			shift
			;;
		-r | --openwrt-repository)
			OPENWRT_REPOSITORY="$2"
			shift
			shift
			;;
		-s | --shell)
	...
	*)
	...
	if [[ " ${legacy_platforms[*]} " =~ " $TARGET_DEVICE " ]]; then
		...
		dbg "$TARGET_DEVICE non-legacy platform, building on prplOS latest-24.10_2025-09-23 UPDK-9.2.0"
		OPENWRT_TOOLCHAIN_VERSION='72689c9980407799b9040bfa2e4648a51d048c6a'
		OPENWRT_VERSION='72689c9980407799b9040bfa2e4648a51d048c6a'
	fi
...
# prplos latest-24.10_2025-09-23
OPENWRT_TOOLCHAIN_VERSION='72689c9980407799b9040bfa2e4648a51d048c6a'
OPENWRT_VERSION='72689c9980407799b9040bfa2e4648a51d048c6a'
