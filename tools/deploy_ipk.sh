...
deploy() {
    ...
    # -O needs to be added for recent OpenSSL versions on the host, use scp instead of sftp
    OPENSSL_MAJOR=$(openssl version | awk '{print substr($2,1,1)}')
    SCP_LEGACY_FLAG="-O"
    if [ "$OPENSSL_MAJOR" -eq 1 ]; then SCP_LEGACY_FLAG=""; fi
