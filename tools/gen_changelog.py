...
def parse_merge_commit(logline: str, items: list):
    ...
    item = re.match("([0-9a-f]+) Merge branch '(bugfix|feature)/(PPM-\\d+|PPM_\\d+|PPW-\\d+|PPW_\\d+|PCF-\\d+|PCF_\\d+)",
                    logline)
    if not item:
...
def main():
    ...
    tags = subprocess.run(['git', 'tag', '--merged', get_branchname("HEAD")], capture_output=True, text=True).stdout
