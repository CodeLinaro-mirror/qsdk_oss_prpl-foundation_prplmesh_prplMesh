 ...
 */
constexpr auto vbss_deauth_unknown_stas_grace_period =
...
static void
copy_vaps_info_and_type(std::shared_ptr<bwl::ap_wlan_hal> &ap_wlan_hal,
  ...
  // Copy the VAPs
  for (int vap_id = beerocks::IFACE_VAP_ID_MIN, i = 0;
       vap_id <= beerocks::IFACE_VAP_ID_MAX; vap_id++, i++) {
    ...
    beerocks::string_utils::copy_string(
        vap_types[i].vap_label, curr_vap.vap_label.c_str(),
        beerocks::message::VAP_LABEL_MAX_LENGTH);
  }
}

static void build_channels_list(
  ...
  // Copy the Rank from the channels list to a helper container "ranks" that
  // will be used to convert the rank to multi-ap preference.
  for (const auto &channel_element : channels_list) {
    ...
    for (const auto &bw_info : channel_info.bw_info_list) {
      ...
      // Copy rank to helper container that will help to convert the rank to
      // multi-ap preference
      ranks[rank].insert(rank);
    }
  }

  // Multi-AP allows only 15 options of preference whereas the ranking from the
  // ACS-Report has 2^31 options. To scale the rank to 1-14 (0 is not operable
  // and represented by a rank of  -1), group rankings with small delta until
  // only 14 groups are left: {group_rank_average, { rank1, rank2, rank3}}.
  // Note: Since "std::map" is ordered container, we don't have to sort it.
  LOG(DEBUG) << "Narrow ranks to groups, ranks.size()=" << ranks.size();
  constexpr uint8_t max_score =
      wfa_map::cPreferenceOperatingClasses::ePreference::PREFERRED14;
  while (ranks.size() > max_score) {
    ...
    // Unify the original ranks which are under the same group, and add the
    // unified group to the ranks list. The two separated groups that that made
    // the new group are removed.
    for (const auto &min_delta_ranks_group : min_delta_ranks_groups) {
      ...
      auto average_rank = std::accumulate(unified_rank_elements.begin(),
                                          unified_rank_elements.end(), 0) /
  ...
  for (const auto &channel_info_pair : channels_list) {
    ...
    channel_info_tlv->dfs_state() =
        [](const beerocks::eDfsState bwl_dfs_state) {
    ...
    for (uint8_t i = 0; i < channel_info_tlv->supported_bandwidths_length();
         i++) {
      auto &supported_bw_info_tlv =
          std::get<1>(channel_info_tlv->supported_bandwidths(i));
      ...
      auto print_channel_info = [&]() {
        ...
        LOG(DEBUG) << "channel=" << int(channel_info_pair.first) << ", bw="
                   ...
                   << ", rank=" << supported_bw_info_tlv.rank
                   << ", multiap_preference="
                   << int(supported_bw_info_tlv.multiap_preference)
                   << ", dfs_state="
                   << dfs_state_to_string(channel_info_tlv->dfs_state());
      };

      // If channel's DFS State is unavailable, set the channel preference to
      // "Not Usable" (0).
      if (channel_info_tlv->dfs_state() ==
      ...
      // The ranks are sorted since they are on an ordered container. Therefore,
      // use the the index of each element to calculate the Multi-AP preference
      // by subtracting the rank element index from 15 (Best rank).
      for (uint8_t rank_idx = 0; rank_idx < ranks.size(); rank_idx++) {
        auto &rank_group_set = std::next(ranks.begin(), rank_idx)->second;
        if (rank_group_set.find(supported_bw_info_tlv.rank) !=
            rank_group_set.end()) {
...
auto ApManager::sApManagerState::operator=(eApManagerState state)
    -> eApManagerState {
  cur = state;
...
ApManager::ApManager(
    ...
    : cmdu_tx(m_tx_buffer, sizeof(m_tx_buffer)), m_logger(logger),
      ...
      m_slave_cmdu_client_factory(slave_cmdu_client_factory),
      m_timer_manager(timer_manager), m_event_loop(event_loop) {
  LOG_IF(!m_slave_cmdu_client_factory, FATAL)
...
bool ApManager::create_ap_wlan_hal() {
  ...
  if (!beerocks::bpl::bpl_cfg_get_hostapd_ctrl_path(m_iface,
                                                    hal_conf.wpa_ctrl_path)) {
    LOG(ERROR) << "Couldn't get hostapd control path for interface " << m_iface;
    return false;
  }

  if (!beerocks::bpl::bpl_cfg_get_monitored_BSSs_by_radio_iface(
          m_iface, hal_conf.monitored_BSSs)) {
    LOG(DEBUG) << "Failed to get radio-monitored-BSSs for interface "
               << m_iface;
  }

  // Create a new AP HAL instance
  ap_wlan_hal = bwl::ap_wlan_hal_create(
      m_iface, hal_conf, std::bind(&ApManager::hal_event_handler, this, _1));
...
bool ApManager::start() {
  ...
  // In case of error in one of the steps of this method, we have to undo all
  // the previous steps (like when rolling back a database transaction, where
  // either all steps get executed or none of them gets executed)
  beerocks::Transaction transaction;

  // Create a timer to run the FSM periodically
  m_fsm_timer = m_timer_manager->add_timer(
      "AP Manager FSM", fsm_timer_period, fsm_timer_period,
  ...
  transaction.add_rollback_action(
      [&]() { m_timer_manager->remove_timer(m_fsm_timer); });

  // Create an instance of a CMDU client connected to the CMDU server that is
  ...
  // Install a CMDU-received event handler for CMDU messages received from the
  // slave.
  ...
  handlers.on_connection_closed = [&]() {
    ...
    // Don't put here a "m_slave_client.reset()" since it will destruct this
...
bool ApManager::ap_manager_fsm(bool &continue_processing) {
  ...
  case eApManagerState::INIT: {
    auto request = message_com::create_vs_message<
        beerocks_message::cACTION_APMANAGER_UP_NOTIFICATION>(cmdu_tx);
    ...
    m_state_timeout = std::chrono::steady_clock::now() +
                      std::chrono::milliseconds(SELECT_TIMEOUT_MSC);
    break;
  }
  case eApManagerState::WAIT_FOR_CONFIGURATION: {
    // On ACTION_APMANAGER_CONFIGURE handler, the ap_hal will be created, and
    // the state will change to ATTACHING state.
  ...
  case eApManagerState::ATTACHING: {
    ...
    LOG(INFO) << "waiting to attach to "
              << ap_wlan_hal->get_radio_info().iface_name;
    break;
  }
  case eApManagerState::ATTACHED: {
    ...
    if (ext_event_fd_max == 0) {
      LOG(DEBUG) << "No external event FD is available, periodic polling will "
                    "be done instead.";
    } else if (ext_event_fd_max < 0) {
      LOG(ERROR) << "Invalid external event file descriptors: "
                 << ext_event_fd_max;
      return false;
    }
...
bool ApManager::handle_aps_update_list()
{
    ...
    if (notification == nullptr) {
      ...
      LOG(ERROR) << "Invalid internal event file descriptor: "
                 << m_ap_hal_int_events;
      return false;
    }
  ...
  case eApManagerState::OPERATIONAL: {
    ...
    auto itFdMax = std::max_element(std::begin(m_ap_hal_ext_events),
                                    std::end(m_ap_hal_ext_events));
    ...
    // Long running operations prevent the event loop from doing anything else
    // (i.e.: the event loop is not able to react to any incoming request in the
    // meantime). Therefore, limit the maximum amount of time that the following
    // method can run, instead of letting it run non-stop for what could be
    // quite a long time if there are many clients already connected.
    auto max_iteration_timeout =
        std::chrono::steady_clock::now() + fsm_timer_period / 2;

    if (m_generate_connected_clients_events &&
        (m_next_generate_connected_events_time <
         std::chrono::steady_clock::now())) {
      bool is_finished_all_clients = false;
      // If there is not enough time to generate all events, the method will be
      // called in the next FSM iteration, and so on until all connected clients
      // are eventually reported.
      auto max_generate_timeout =
          (std::chrono::steady_clock::now() +
           std::chrono::milliseconds(
               GENERATE_CONNECTED_EVENTS_WORK_TIME_LIMIT_MSEC));
      max_generate_timeout =
          std::min(max_generate_timeout, max_iteration_timeout);
      // If there is not enough time to generate all events, the method will be
      // called in the next FSM iteration, and so on until all connected clients
      // are eventually reported.
      if (!ap_wlan_hal->generate_connected_clients_events(
              is_finished_all_clients, max_generate_timeout)) {
...
void ApManager::handle_cmdu_ieee1905_1_message(
    ieee1905_1::CmduMessageRx &cmdu_rx) {
  ...
  case ieee1905_1::eMessageType::
      TRIGGER_CHANNEL_SWITCH_ANNOUNCEMENT_REQUEST_MESSAGE: {
    handle_trigger_channel_switch_announcement_request(cmdu_rx);
    break;
  }

  default:
    LOG(ERROR) << "Unknown CMDU message type: " << std::hex
               << int(cmdu_message_type);
  }
}

void ApManager::handle_dpp_cce_indication_message(
    ieee1905_1::CmduMessageRx &cmdu_rx) {
  const auto mid = cmdu_rx.getMessageId();
  auto dpp_cce_indication_tlv =
      cmdu_rx.getClass<wfa_map::tlvDppCceIndication>();
...
void ApManager::handle_direct_encap_dpp_message(
    ieee1905_1::CmduMessageRx &cmdu_rx) {
  const auto mid = cmdu_rx.getMessageId();
  ...
  // forwarding of dpp authentication response message to BWL layer to be
  // implemented.
}
...
void ApManager::handle_virtual_bss_request(ieee1905_1::CmduMessageRx &cmdu_rx) {
  ...
  auto send_virtual_bss_response = [this](const sMacAddr &radio_uid,
                                          const sMacAddr &bssid, bool success) {
    auto cmdu_tx_header = cmdu_tx.create(
        0, ieee1905_1::eMessageType::VIRTUAL_BSS_RESPONSE_MESSAGE);
    if (!cmdu_tx_header) {
      LOG(ERROR)
          << "cmdu creation of type VIRTUAL_BSS_RESPONSE_MESSAGE failed!";
      return;
    }
  ...
  auto virtual_bss_creation_tlv =
      cmdu_rx.getClass<wfa_map::VirtualBssCreation>();

  if (virtual_bss_creation_tlv) {

    std::string ifname =
        get_vbss_interface_name(virtual_bss_creation_tlv->bssid());
    std::string bridge = "br-lan";

    // TODO: PPM-2348 add the bridge name to BPL

    // TODO: the VirtualBSSCreation TLV doesn't specify authentication and
    // encryption types
    ...
    if (virtual_bss_creation_tlv->client_assoc()) {
      LOG(DEBUG) << "The client was already associated, adding a new station "
                    "and its keys.";
      ...
      if (!ap_wlan_hal->sta_acceptlist_modify(
              virtual_bss_creation_tlv->client_mac(),
              virtual_bss_creation_tlv->bssid(), bwl::sta_acl_action::ADD)) {
      ...
      if (!ap_wlan_hal->set_beacon_da(ifname,
                                      virtual_bss_creation_tlv->client_mac())) {
      ...
      auto client_capability_report =
          cmdu_rx.getClass<wfa_map::tlvClientCapabilityReport>();
      if (!client_capability_report) {
        LOG(ERROR)
            << "Missing client capability report for an associated client!";
        send_virtual_bss_response(virtual_bss_creation_tlv->radio_uid(),
                                  virtual_bss_creation_tlv->bssid(), false);
        return;
      }
      ...
      std::vector<uint8_t> raw_association_frame = {
          ...
          client_capability_report->association_frame() +
              client_capability_report->association_frame_length()};
      if (!ap_wlan_hal->add_station(ifname,
                                    virtual_bss_creation_tlv->client_mac(),
      ...
      // source agent.
      auto insert_rx_pn = [](std::vector<uint8_t> &key_seq,
                             size_t len) -> void {
      ...
      bwl::sKeyInfo pairwise_key = {
          ...
          .key = {virtual_bss_creation_tlv->ptk(),
                  virtual_bss_creation_tlv->ptk() +
                      virtual_bss_creation_tlv->key_length()},
          .key_seq = pw_key_seq,

          // TODO: PPM-2368: We need to know the pairwise cipher. For now, use
          // CCMP
      ...
      bwl::sKeyInfo group_key = {
          ...
          .key = {virtual_bss_creation_tlv->gtk(),
                  virtual_bss_creation_tlv->gtk() +
                      virtual_bss_creation_tlv->key_length()},
          .key_seq = group_key_seq,

          // TODO: PPM-2368: We need to know the groupwise cipher. For now, use
          // CCMP
          .key_cipher = 0x000FAC04};
    ...
    m_vbss_deauth_unknown_stas_timer = m_timer_manager->add_timer(
        ...
        [=](int fd, beerocks::EventLoop &loop) {
          ...
          if (!ap_wlan_hal->set_no_deauth_unknown_sta(ifname, false)) {
            LOG(WARNING) << "Failed to set no_deauth_unknown_sta! ifname: "
                         << ifname;
          }
          return true;
        });

    if (m_vbss_deauth_unknown_stas_timer ==
  ...
  auto virtual_bss_destruction_tlv =
      cmdu_rx.getClass<wfa_map::VirtualBssDestruction>();
  if (virtual_bss_destruction_tlv) {

    std::string ifname =
        get_vbss_interface_name(virtual_bss_destruction_tlv->bssid());
    ...
    if (virtual_bss_destruction_tlv->disassociate_client()) {
      ...
      // There is no station MAC included in the virtual BSS destruction TLV,
      // and there is no convenient way to get a list of the stations connected
      // to a VAP, so nuke all associated stations (since the BSS will
      // subsequently be removed anyway).
      if (!ap_wlan_hal->sta_disassoc(
              vap_id, beerocks::net::network_utils::WILD_MAC_STRING)) {
  ...
  LOG(ERROR) << "No virtual BSS creation nor destruction TLV found in Virtual "
                "BSS request!";
}

void ApManager::handle_virtual_bss_move_preparation_request(
    ieee1905_1::CmduMessageRx &cmdu_rx) {
  ...
  if (!cmdu_tx_header) {
    LOG(ERROR) << "cmdu creation of type "
                  "VIRTUAL_BSS_MOVE_PREPARATION_RESPONSE_MESSAGE failed!";
    return;
  }
  ...
  if (!ap_wlan_hal->send_delba(
          get_vbss_interface_name(client_info_tlv_tx->bssid()),
          client_info_tlv_rx->client_mac(), client_info_tlv_tx->bssid(),
          client_info_tlv_tx->bssid())) {
    LOG(ERROR) << "Failed to send DELBA to '"
...
void ApManager::handle_virtual_bss_move_cancel_request(
    ieee1905_1::CmduMessageRx &cmdu_rx) {
  ...
  auto cmdu_tx_header = cmdu_tx.create(
      0, ieee1905_1::eMessageType::VIRTUAL_BSS_MOVE_CANCEL_RESPONSE_MESSAGE);
  if (!cmdu_tx_header) {
    LOG(DEBUG) << "cmdu creation of type "
                  "VIRTUAL_BSS_MOVE_CANCEL_RESPONSE_MESSAGE failed!";
    return;
  }
  ...
  // The VBSS spec leaves the actions needed to handle a Move Cancel up to the
  // Agent.
...
void ApManager::handle_unassoc_sta_link_metrics_query_message(
    ieee1905_1::CmduMessageRx &cmdu_rx) {
  const auto mid = cmdu_rx.getMessageId();
  LOG(DEBUG) << "Received UNASSOCIATED_STA_LINK_METRICS_QUERY_MESSAGE, mid="
             << std::hex << mid;
  ...
  if (!unassoc_sta_link_metric_query) {
    LOG(ERROR) << "Unassoc sta link metric query cmdu mid =" << mid
               << " get class failed";
    return;
  }
  ...
  send_cmdu(cmdu_tx);
  ap_wlan_hal->send_unassoc_sta_link_metric_query(
      unassoc_sta_link_metric_query);
}
...
void ApManager::handle_cmdu(ieee1905_1::CmduMessageRx &cmdu_rx) {
  ...
  if (ap_wlan_hal && (beerocks_header->action_op() ==
                      beerocks_message::ACTION_APMANAGER_CONFIGURE)) {
  ...
  case beerocks_message::ACTION_APMANAGER_CONFIGURE: {
    ...
    auto config =
        beerocks_header
            ->addClass<beerocks_message::cACTION_APMANAGER_CONFIGURE>();
    if (!config) {
  ...
  case beerocks_message::ACTION_APMANAGER_ENABLE_APS_REQUEST: {
    ...
    auto notification = beerocks_header->addClass<
        beerocks_message::cACTION_APMANAGER_ENABLE_APS_REQUEST>();
    if (!notification) {
    ...
    auto response = message_com::create_vs_message<
        beerocks_message::cACTION_APMANAGER_ENABLE_APS_RESPONSE>(cmdu_tx);
    ...
    // Disable the radio interface to make hostapd to consider the new
    // configuration.
    if (!ap_wlan_hal->disable()) {
      LOG(DEBUG) << "ap disable() failed!, the interface might be already "
                    "disabled or down";
    }
#endif

    // If it is not the radio of the BH, then channel, bandwidth and center
    // channel paramenters will be all set to 0.
    ...
    if (!ap_wlan_hal->set_channel(notification->channel(),
  ...
  case beerocks_message::
    ...
    if (request == nullptr) {
      LOG(ERROR) << "addClass "
                    "cACTION_APMANAGER_HOSTAP_SET_RESTRICTED_FAILSAFE_CHANNEL_"
                    "REQUEST failed";
      return;
    }
    ...
    if (!ap_wlan_hal->restricted_channels_set(
            (char *)request->params().restricted_channels)) {
    ...
    if (failsafe_channel != 0) {
      ...
      LOG(INFO) << " Calling failsafe_channel_set - "
                ...
                << ", channel_bandwidth: "
                << beerocks::utils::convert_bandwidth_to_string(
                       (beerocks::eWiFiBandwidth)request->params()
                           .failsafe_channel_bandwidth)
                << ", vht_center_frequency: "
                << request->params().vht_center_frequency;

      if (!ap_wlan_hal->failsafe_channel_set(
              failsafe_channel, request->params().failsafe_channel_bandwidth,
              request->params().vht_center_frequency)) {
    ...
    LOG(INFO) << "send "
                 "ACTION_APMANAGER_HOSTAP_SET_RESTRICTED_FAILSAFE_CHANNEL_"
                 "RESPONSE success = "
              << int(success);
    auto response = message_com::create_vs_message<
        beerocks_message::
  ...
  case beerocks_message::ACTION_APMANAGER_HOSTAP_CHANNEL_SWITCH_ACS_START: {
    ...
    if (request == nullptr) {
      LOG(ERROR) << "addClass "
                    "cACTION_APMANAGER_HOSTAP_CHANNEL_SWITCH_ACS_START failed";
      return;
    }
    LOG(DEBUG) << "ACTION_APMANAGER_HOSTAP_CHANNEL_SWITCH_ACS_START: requested "
                  "channel="
    ...
    if (request->spatial_reuse_valid()) {
      ...
      spatial_reuse_params.partial_bss_color =
          request->sr_params().partial_bss_color;
      spatial_reuse_params.hesiga_spatial_reuse_value15_allowed =
          request->sr_params().hesiga_spatial_reuse_value15_allowed;
      spatial_reuse_params.srg_information_valid =
          request->sr_params().srg_information_valid;
      spatial_reuse_params.non_srg_offset_valid =
          request->sr_params().non_srg_offset_valid;
      ...
      spatial_reuse_params.srg_obsspd_min_offset =
          request->sr_params().srg_obsspd_min_offset;
      spatial_reuse_params.srg_obsspd_max_offset =
          request->sr_params().srg_obsspd_max_offset;
      spatial_reuse_params.srg_bss_color_bitmap =
          request->sr_params().srg_bss_color_bitmap;
      ...
      // Starts a timer to trigger a CSA notification after approximately 5
      // seconds. The ap_manager continues executing other tasks while the timer
      // is ongoing. When the timer elapses, the ap_manager performs the
      // following actions:
      //  1. Uses the HAL API to get the spatial reuse configuration.
      //  2. Sends the
      //  cACTION_APMANAGER_HOSTAP_SPATIAL_REUSE_REPORT_NOTIFICATION event
      //     to the son_slave_thread, which adds the fetched values to the radio
      //     structure of AgentDB.
      //  3. Sends the cACTION_APMANAGER_HOSTAP_CSA_NOTIFICATION event to the
      //  son_slave_thread
    ...
    if (request->tx_limit_valid()) {
      ...
      if (ap_wlan_hal->get_radio_info().channel ==
        ...
        if (!csa_notification_timer_on()) {
          // If csa_notification_timer is enabled, a color switch event (BSS
          // Color) is going on and the CSA which is required to be sent here
          // will be handled when the timer elapses.
          LOG(DEBUG) << "Sending CSA notification since csa_notification_timer "
                        "is disabled";
          auto notification = message_com::create_vs_message<
              beerocks_message::cACTION_APMANAGER_HOSTAP_CSA_NOTIFICATION>(
              cmdu_tx);
          if (!notification) {
            LOG(ERROR) << "Failed building message!";
            return;
    ...
    if ((!request->cs_params().channel) ||
      ...
      LOG(INFO) << "No need to switch channels as current channel and "
                   "requested channels are "
                   "the same.";
      return;
    }
  ...
  case beerocks_message::ACTION_APMANAGER_HOSTAP_CANCEL_ACTIVE_CAC_REQUEST: {
    auto request = beerocks_header->addClass<
        beerocks_message::cACTION_APMANAGER_HOSTAP_CANCEL_ACTIVE_CAC_REQUEST>();
    if (request == nullptr) {
      LOG(ERROR) << "addClass "
                    "cACTION_APMANAGER_HOSTAP_CANCEL_ACTIVE_CAC_REQUEST failed";
      return;
    }
    ...
    // In order to make sure cac was actually canceled and radio is operational
    // again we need to poll until radio state is ENABLED or DFS (in case radio
    // is enabled back on DFS channel) or timeout. NOTE: bwl implementations
    // default radio_state == UNKNOWN so consider them also as ENABLED.
    auto timeout = std::chrono::steady_clock::now() +
                   std::chrono::seconds(MAX_CANCEL_CAC_TIMEOUT_SEC);
    while (cancel_cac_success && std::chrono::steady_clock::now() < timeout) {
      ...
      if (ap_wlan_hal->get_radio_info().radio_state ==
              bwl::eRadioState::ENABLED ||
          (ap_wlan_hal->get_radio_info().radio_state ==
           bwl::eRadioState::DFS) ||
          (ap_wlan_hal->get_radio_info().radio_state ==
           bwl::eRadioState::UNKNOWN)) {
    ...
    // As part of cancel CAC flow hostapd is re-enabled on diffrent (previous)
    // channel and hostapd may not send CSA (channel switch notification) event.
    // Generate CSA notification anyway and send to upper layers to make
    // sure agent DB is up to date.
    if (response->success()) {
      auto notification = message_com::create_vs_message<
  ...
  case beerocks_message::ACTION_APMANAGER_HOSTAP_SET_NEIGHBOR_11K_REQUEST: {
    LOG(WARNING)
        << "UNIMPLEMENTED - ACTION_APMANAGER_HOSTAP_SET_NEIGHBOR_11K_REQUEST";
    // auto request =
    // (message::sACTION_APMANAGER_HOSTAP_SET_NEIGHBOR_11K_REQUEST*)rx_buffer;
  ...
  case beerocks_message::ACTION_APMANAGER_HOSTAP_REMOVE_NEIGHBOR_11K_REQUEST: {
    LOG(WARNING) << "UNIMPLEMENTED - "
                    "ACTION_APMANAGER_HOSTAP_REMOVE_NEIGHBOR_11K_REQUEST";
    // auto request =
    // (message::sACTION_APMANAGER_HOSTAP_REMOVE_NEIGHBOR_11K_REQUEST*)rx_buffer;
  ...
  case beerocks_message::ACTION_APMANAGER_CLIENT_DISCONNECT_REQUEST: {
    ...
    auto request = beerocks_header->addClass<
        beerocks_message::cACTION_APMANAGER_CLIENT_DISCONNECT_REQUEST>();
    if (request == nullptr) {
      LOG(ERROR)
          << "addClass cACTION_APMANAGER_CLIENT_DISCONNECT_REQUEST failed";
      send_steering_return_status(
          beerocks_message::ACTION_APMANAGER_CLIENT_DISCONNECT_RESPONSE,
          OPERATION_FAIL);
      return;
    }
    ...
    LOG(DEBUG) << "CLIENT_DISCONNECT, type "
               << ((type == beerocks_message::eDisconnect_Type_Deauth)
                       ? "DEAUTH"
                       : "DISASSOC")
    ...
    auto response = message_com::create_vs_message<
        beerocks_message::cACTION_APMANAGER_CLIENT_DISCONNECT_RESPONSE>(
        cmdu_tx);
    if (response == nullptr) {
      LOG(ERROR) << "Failed building message!";
      return;
  ...
  case beerocks_message::ACTION_APMANAGER_CLIENT_DISALLOW_REQUEST: {
    auto request = beerocks_header->addClass<
        beerocks_message::cACTION_APMANAGER_CLIENT_DISALLOW_REQUEST>();
    if (request == nullptr) {
    ...
    for (auto i = 0; i < sta_mac_list_length; i++) {
      ...
      const auto &vap_unordered_map =
          ap_wlan_hal->get_radio_info().available_vaps;
      auto it = std::find_if(
      ...
      if (it == vap_unordered_map.end()) {
        // AP does not have the requested vap, probably will be handled on the
        // other AP
        return;
      }

      LOG(DEBUG) << "CLIENT_DISALLOW: mac = "
                 << tlvf::mac_to_string(sta_info.mac)
      ...
      // Check if validity period is set then add it to the "disallowed client
      // timeouts" list This list will be polled in ap_manager_fsm() while in
      // operational state through method `allow_expired_clients()` When
      // validity period is timed out sta_allow will be called.
      if (request->validity_period_sec()) {
        disallowed_client_t disallowed_client;

        // calculate new disallow timeout from client validity period parameter
        // [sec]
        disallowed_client.timeout =
            std::chrono::steady_clock::now() +
        ...
        LOG(DEBUG) << "client " << disallowed_client.mac
                   << " will be allowed to associate with bssid "
                   << disallowed_client.bssid << " in "
                   << std::chrono::duration_cast<std::chrono::seconds>(
                          disallowed_client.timeout -
                          std::chrono::steady_clock::now())
                          .count()
        ...
        LOG(WARNING) << "CLIENT_DISALLOW validity period set to 0, STA mac "
                     << sta_info.mac << " will remain blocked from bssid "
                     << request->bssid();
      }
    }
    break;
  ...
  case beerocks_message::ACTION_APMANAGER_CLIENT_ALLOW_REQUEST: {
    auto request = beerocks_header->addClass<
        beerocks_message::cACTION_APMANAGER_CLIENT_ALLOW_REQUEST>();
    if (request == nullptr) {
    ...
    for (auto i = 0; i < sta_mac_list_length; i++) {
      ...
      remove_client_from_disallowed_list(std::get<1>(sta_tuple),
                                         request->bssid());

      LOG(DEBUG) << "CLIENT_ALLOW: mac = "
                 << tlvf::mac_to_string(std::get<1>(sta_tuple))
                 << ", bssid = " << tlvf::mac_to_string(request->bssid());
  ...
  case beerocks_message::ACTION_APMANAGER_CHANNELS_LIST_REQUEST: {
    ...
    // Refreshing the radio info updates the DFS State of each channel on the
    // channels list.
    if (!ap_wlan_hal->refresh_radio_info()) {
    ...
    if (!ap_wlan_hal->read_acs_report()) {
      ...
      // If the ACS report is available, it is used to evaluate the ranks of the
      // channels. If it is not available or fails, the default procedure is
      // followed. In either case, the function does not return early even if
      // read_acs_report fails.
      LOG(ERROR) << "Failed to read acs report";
    }

    auto response = beerocks::message_com::create_vs_message<
        beerocks_message::cACTION_APMANAGER_CHANNELS_LIST_RESPONSE>(
        cmdu_tx, beerocks_header->id());
    if (!response) {
      LOG(ERROR) << "Failed building message!";
      return;
  ...
  case beerocks_message::ACTION_APMANAGER_CLIENT_RX_RSSI_MEASUREMENT_REQUEST: {
    ...
    if (request == nullptr) {
      LOG(ERROR)
          << "addClass cACTION_APMANAGER_CLIENT_RX_RSSI_MEASUREMENT_REQUEST "
             "failed";
      return;
    }
    ...
    if (sta_unassociated_rssi_measurement_header_id == -1) {
      ...
      send_cmdu(cmdu_tx);
      LOG(DEBUG)
          << "send sACTION_APMANAGER_CLIENT_RX_RSSI_MEASUREMENT_CMD_RESPONSE, "
             "sta_mac = "
          << sta_mac;
      ...
      if (ap_wlan_hal->sta_unassoc_rssi_measurement(
              ...
              request->params().vht_center_frequency,
              request->params().measurement_delay,
      ...
      LOG(DEBUG) << "CLIENT_RX_RSSI_MEASUREMENT_REQUEST, mac = " << sta_mac
                 ...
                 << " vht_center_frequency = "
                 << int(request->params().vht_center_frequency)
                 << " id = " << sta_unassociated_rssi_measurement_header_id;
    } else {
      ap_busy = true;
      LOG(WARNING) << "busy!, send response to retry "
                      "CLIENT_RX_RSSI_MEASUREMENT_REQUEST, mac = "
                   << sta_mac;
    }
  ...
  case beerocks_message::ACTION_APMANAGER_CLIENT_BSS_STEER_REQUEST: {
    auto request = beerocks_header->addClass<
        beerocks_message::cACTION_APMANAGER_CLIENT_BSS_STEER_REQUEST>();
    if (request == nullptr) {
      LOG(ERROR)
          << "addClass cACTION_APMANAGER_CLIENT_BSS_STEER_REQUEST failed";
      return;
    }
    ...
    auto response =
        message_com::create_vs_message<beerocks_message::cACTION_APMANAGER_ACK>(
            cmdu_tx, beerocks_header->id());
    ...
    btm_params.pref_list_included =
        true; // since a valid target BSSID is always assumed
  ...
  case beerocks_message::ACTION_APMANAGER_WIFI_CREDENTIALS_UPDATE_REQUEST: {
    ...
    if (!request) {
      LOG(ERROR)
          << "addClass ACTION_APMANAGER_WIFI_CREDENTIALS_UPDATE_REQUEST failed";
      return;
    }
    ...
    for (auto i = 0; i < wifi_credentials_size; i++) {
      ...
      // If a Multi-AP Agent receives an AP-Autoconfiguration WSC message
      // containing an M2 with a Multi-AP Extension subelement with bit 4 (Tear
      // Down bit) of the subelement value set to one (see Table 4), it shall
      // tear down all currently operating BSS(s) on the radio indicated by the
      // Radio Unique Identifier, and shall ignore the other attributes in the
      // M2.
      auto bss_type = static_cast<WSC::eWscVendorExtSubelementBssType>(
          config_data.bss_type());
      ...
      if ((bss_type & WSC::eWscVendorExtSubelementBssType::FRONTHAUL_BSS) !=
          0) {
      ...
      bss_info_conf.profile1_backhaul_sta_association_disallowed =
          bss_type & WSC::eWscVendorExtSubelementBssType::
      ...
      bss_info_conf.authentication_type =
          config_data.authentication_type_attr().data;
      ...
      bss_info_conf.additional_auth =
          static_cast<son::wireless_utils::eAdditionalAuth>(
              config_data.additional_auth());
    ...
    while ((std::chrono::steady_clock::now() < timeout) && !radio_state_lock) {
      ...
      if (ap_wlan_hal->get_radio_info().radio_state ==
        ...
        perform_update = true;
        LOG(DEBUG)
            << "Radio is in enabled state, performing vap credentials update";
        break;
      }
      UTILS_SLEEP_MSEC(500);
    }
    ...
    if (perform_update && !bss_info_conf_list.empty()) {
      ...
      auto vap_timeout =
          std::chrono::steady_clock::now() + wait_for_vaps_enable_timeout_sec;
      ...
      while (std::chrono::steady_clock::now() < vap_timeout) {
        ...
        if (!all_vaps_enabled &&
            ap_wlan_hal->get_vap_status(bss_info_conf_list)) {
          LOG(INFO) << "All vaps are enabled";
          all_vaps_enabled = true;
        }

        if (!all_mld_updated &&
            ap_wlan_hal->update_mld_status(bss_info_conf_list)) {
      ...
      // then radio tx power is updated, so to keep agent DB updated send CSA
      // notification like it is handled in
      ...
      // The 'available_vaps' container is cleared and filled inside
      // update_vap_credentials()
      auto response = message_com::create_vs_message<
          beerocks_message::cACTION_APMANAGER_WIFI_CREDENTIALS_UPDATE_RESPONSE>(
          cmdu_tx);
      const auto &radio_vaps = ap_wlan_hal->get_radio_info().available_vaps;
      ...
      LOG(ERROR) << "discard new configuration because "
                 << (perform_update ? "configuration is empty"
                                    : "radio not enabled");
    }
    break;
  }
  ...
  case beerocks_message::ACTION_APMANAGER_SET_ASSOC_DISALLOW_REQUEST: {
    ...
    auto request = beerocks_header->addClass<
        beerocks_message::cACTION_APMANAGER_SET_ASSOC_DISALLOW_REQUEST>();
    if (!request) {
      LOG(ERROR)
          << "addClass ACTION_APMANAGER_SET_ASSOC_DISALLOW_REQUEST failed";
      return;
    }
  ...
  case beerocks_message::ACTION_APMANAGER_RADIO_DISABLE_REQUEST: {
    ...
    if (ap_wlan_hal->get_radio_info().radio_state ==
        bwl::eRadioState::DISABLED) {
      return;
    }

    // Disable the radio interface
    if (!ap_wlan_hal->disable()) {
      LOG(ERROR) << "Failed disabling radio on iface: "
  ...
  case beerocks_message::ACTION_APMANAGER_RADIO_ENABLE_REQUEST: {
    ...
    if (!ap_wlan_hal->enable()) {
      LOG(ERROR) << "Failed enabling radio on iface: "
                 << ap_wlan_hal->get_iface_name();
      return;
    }
  ...
  case beerocks_message::ACTION_APMANAGER_RADIO_MODE_CONFIG_REQUEST: {
    auto request = beerocks_header->addClass<
        beerocks_message::cACTION_APMANAGER_RADIO_MODE_CONFIG_REQUEST>();
    if (!request) {
      LOG(ERROR)
          << "addClass cACTION_APMANAGER_RADIO_MODE_CONFIG_REQUEST failed";
      return;
    }

    if (!ap_wlan_hal->change_radio_mode_config(
            request->operating_standards())) {
  ...
  case beerocks_message::ACTION_APMANAGER_STEERING_CLIENT_SET_REQUEST: {
    auto request = beerocks_header->addClass<
        beerocks_message::cACTION_APMANAGER_STEERING_CLIENT_SET_REQUEST>();
    if (request == nullptr) {
      LOG(ERROR)
          << "addClass cACTION_APMANAGER_STEERING_CLIENT_SET_REQUEST failed";
      send_steering_return_status(
    ...
    LOG(DEBUG) << "Got ACTION_APMANAGER_STEERING_CLIENT_SET_REQUEST for BSSID="
               << bssid << " VAP=" << vap_name;
    if (!request->params().remove &&
        (request->params().config.snrProbeHWM > 0)) {
      if (!ap_wlan_hal->sta_softblock_add(
              vap_name, tlvf::mac_to_string(request->params().client_mac),
              request->params().config.authRejectReason,
              request->params().config.snrProbeHWM,
              request->params().config.snrProbeLWM,
              request->params().config.snrAuthHWM,
  ...
  case beerocks_message::
    ...
    if (!ap_wlan_hal->pre_generate_connected_clients_events()) {
      LOG(WARNING)
          << "Failed to prepare for generate of clients connected events";
    }
    break;
  }
  case beerocks_message::
      ACTION_APMANAGER_HOSTAP_ZWDFS_ANT_CHANNEL_SWITCH_REQUEST: {
    ...
    if (!notification) {
      LOG(ERROR)
          << "addClass "
             "cACTION_APMANAGER_HOSTAP_ZWDFS_ANT_CHANNEL_SWITCH_REQUEST failed";
      return;
    }
    ...
    if (notification->ant_switch_on()) {
      ...
      if (!ap_wlan_hal->switch_channel(
              ...
              notification->center_frequency(), notification->csa_count())) {
      ...
      if (!ap_wlan_hal->is_zwdfs_antenna_enabled()) {
        LOG(WARNING)
            << "trying to switch off zwdfs antenna but its already off!";
    ...
    LOG(INFO)
        << "send cACTION_APMANAGER_HOSTAP_ZWDFS_ANT_CHANNEL_SWITCH_RESPONSE, "
           "success="
        << int(response->success());
    send_cmdu(cmdu_tx);
    break;
  ...
  case beerocks_message::ACTION_APMANAGER_SET_MAP_CONTROLLER_PROFILE: {
    auto msg = beerocks_header->addClass<
        beerocks_message::cACTION_APMANAGER_SET_MAP_CONTROLLER_PROFILE>();
    if (!msg) {
      LOG(ERROR) << "addClass has failed";
      return;
    }
  ...
  case beerocks_message::ACTION_APMANAGER_HOSTAP_SERVICE_PRIO_CONFIG: {
    auto msg = beerocks_header->addClass<
        beerocks_message::cACTION_APMANAGER_HOSTAP_SERVICE_PRIO_CONFIG>();
    if (!msg) {
      LOG(ERROR) << "addClass has failed";
      return;
    }
  ...
  case beerocks_message::ACTION_APMANAGER_MULTI_CHAN_BEACON_11K_REQUEST: {
    ...
    auto msg = beerocks_header->addClass<
        beerocks_message::cACTION_APMANAGER_MULTI_CHAN_BEACON_11K_REQUEST>();
    if (!msg) {
      LOG(ERROR) << "addClass has failed";
      return;
    }
    ...
    beacon_metrics_response.resp_timer = m_timer_manager->add_timer(
        ...
        timer_interval, timer_interval, [&](int fd, beerocks::EventLoop &loop) {
  ...
  case beerocks_message::ACTION_APMANAGER_PLATFORM_ACS: {
    auto request =
        beerocks_header
            ->addClass<beerocks_message::cACTION_APMANAGER_PLATFORM_ACS>();
    if (!request) {
  ...
  case beerocks_message::ACTION_APMANAGER_MLD_UPDATE_REQUEST: {
    auto request = beerocks_header->addClass<
        beerocks_message::cACTION_APMANAGER_MLD_UPDATE_REQUEST>();
    if (!request) {
      LOG(ERROR) << "addClass has failed";
      return;
    }

    LOG(DEBUG) << "handle ACTION_APMANAGER_MLD_UPDATE_REQUEST";

    // Radio is already known in ap_wlan_hal context. What should be updated are
    // the SSIDs with the same ssid of the corresponding radio ONLY as there can
    // be multiple MLDs with the same SSIDs on other radio (ex: single links).
  ...
  case beerocks_message::ACTION_APMANAGER_MLD_MODE_UPDATE_REQUEST: {
    auto request = beerocks_header->addClass<
        beerocks_message::cACTION_APMANAGER_MLD_MODE_UPDATE_REQUEST>();
    if (!request) {
      LOG(ERROR) << "addClass has failed";
      return;
    }
    ...
    // Only SSIDs belonging to the current radio (with matching SSID name) are
    // updated.
    ap_wlan_hal->update_mld_mode(request->ssid(), request->mld_mode());
    break;
  }
  default: {
    LOG(ERROR) << "Unsupported header action_op: "
               << int(beerocks_header->action_op());
    break;
  }
  }
...
void ApManager::beacon_metrics_response_cb(int fd) {
  auto it = std::find_if(m_beacon_metrics_response.begin(),
                         m_beacon_metrics_response.end(),
                         [&fd](const sBeaconMetricsResponse &response) {
                           return response.resp_timer == fd;
                         });
  ...
  auto cmdu_tx_header = cmdu_tx.create(
      0, ieee1905_1::eMessageType::BEACON_METRICS_RESPONSE_MESSAGE);
  if (!cmdu_tx_header) {
    LOG(ERROR)
        << "cmdu creation of type BEACON_METRICS_RESPONSE_MESSAGE failed!";
    m_beacon_metrics_response.erase(it);
    return;
  }

  auto tlvBeaconMetricsResponse =
      cmdu_tx.addClass<wfa_map::tlvBeaconMetricsResponse>();
  ...
  for (const auto &report : response.beacon_report) {
    auto beacon_report =
        tlvBeaconMetricsResponse->create_measurement_report_list();
    if (!beacon_report) {
  ...
  if (response.beacon_report.size() == 0) {
    LOG(DEBUG) << "No beacon report available to be sent for "
               << response.sta_mac;
    ...
    LOG(DEBUG) << response.beacon_report.size()
               << " beacon report(s) available for " << response.sta_mac;
  }
...
void ApManager::fill_sr_params(beerocks_message::sSpatialReuseParams &params) {
  ...
  params.non_srg_obsspd_max_offset =
      spatial_reuse_params.non_srg_obsspd_max_offset;
  ...
  params.srg_partial_bssid_bitmap =
      spatial_reuse_params.srg_partial_bssid_bitmap;
}

bool ApManager::hal_event_handler(
    bwl::base_wlan_hal::hal_event_ptr_t event_ptr) {
  if (!event_ptr) {
  ...
  case Event::CSA_Finished: {
    ...
    LOG(INFO) << ((event == Event::ACS_Completed) ? "ACS_Completed"
                                                  : "CSA_Finished:")
    ...
    if (event == Event::ACS_Completed) {
      ...
      LOG(DEBUG) << "csa_notification_timer isn't enabled. CSA notification "
                    "can be sent";
      auto notification = message_com::create_vs_message<
          beerocks_message::cACTION_APMANAGER_HOSTAP_CSA_NOTIFICATION>(cmdu_tx);
      if (!notification) {
  ...
  case Event::STA_Connected: {
    ...
    auto msg =
        static_cast<bwl::sACTION_APMANAGER_CLIENT_ASSOCIATED_NOTIFICATION *>(
            data);
    ...
    auto notification = message_com::create_vs_message<
        beerocks_message::cACTION_APMANAGER_CLIENT_ASSOCIATED_NOTIFICATION>(
        cmdu_tx);
    if (notification == nullptr) {
      LOG(ERROR) << "Failed building message!";
      return false;
    ...
    auto vap_node =
        ap_wlan_hal->get_radio_info().available_vaps.find(msg->params.vap_id);
    ...
    if (msg->params.num_affiliated_sta > 0) {
      LOG(INFO) << "Processing " << int(msg->params.num_affiliated_sta)
                << " affiliated STAs, notification APMLD="
                << notification->bssid();
      for (size_t idx = 0; idx < msg->params.num_affiliated_sta; ++idx) {
        ...
        affiliated_sta_ptr->mac() =
            msg->params.affiliated_sta[idx].affiliated_sta_mac;

        if (!notification->add_affiliated_sta(affiliated_sta_ptr)) {
          LOG(ERROR) << "Failed to add affiliated_sta[" << idx
                     << "] to notification";
        }
      }
    }
  ...
  case Event::STA_WDS_Iface_Ready: {
    ...
    auto msg =
        static_cast<bwl::sACTION_APMANAGER_WDS_IFACE_NOTIFICATION *>(data);
    ...
    LOG(INFO) << "STA_WDS_Iface_Ready mac = " << msg->params.mac
              << " BSSID = " << msg->params.bssid
              << " iface = " << msg->params.wds_iface_name;
  ...
  case Event::STA_Disconnected: {
    ...
    auto msg =
        static_cast<bwl::sACTION_APMANAGER_CLIENT_DISCONNECTED_NOTIFICATION *>(
            data);
    std::string mac = tlvf::mac_to_string(msg->params.mac);
    LOG(INFO) << "STA_Disconnected client " << mac;

    auto it =
        std::find_if(pending_disable_vaps.begin(), pending_disable_vaps.end(),
                     [&](pending_disable_vap_t vap) {
                       return (vap.vap_id == msg->params.vap_id);
                     });
    ...
    auto notification = message_com::create_vs_message<
        beerocks_message::cACTION_APMANAGER_CLIENT_DISCONNECTED_NOTIFICATION>(
        cmdu_tx);
  ...
  case Event::BSS_TM_Response: {
    ...
    // TODO EasyMesh SteeringBTMReport should contain source BSSID and target
    // BSSID
    auto msg =
        static_cast<bwl::sACTION_APMANAGER_CLIENT_BSS_STEER_RESPONSE *>(data);
  ...
  case Event::STA_Unassoc_RSSI: {
    ...
    auto msg = static_cast<
        bwl::sACTION_APMANAGER_CLIENT_RX_RSSI_MEASUREMENT_RESPONSE *>(data);

    LOG(INFO) << "CLIENT_RX_RSSI_MEASUREMENT_RESPONSE for mac "
              << msg->params.result.mac
              << " id=" << sta_unassociated_rssi_measurement_header_id;
  ...
  case Event::STA_Unassoc_Link_Metrics: {
    ...
    auto cmdu_tx_header =
        cmdu_tx.create(0, ieee1905_1::eMessageType::
                              UNASSOCIATED_STA_LINK_METRICS_RESPONSE_MESSAGE);
    if (!cmdu_tx_header) {
      LOG(ERROR) << "cmdu creation of type "
                    "UNASSOCIATED_STA_LINK_METRICS_RESPONSE_MESSAGE failed!";
      return false;
    }
    ...
    LOG(DEBUG) << "Sending "
                  "ieee1905_1::eMessageType::UNASSOCIATED_STA_LINK_METRICS_"
                  "RESPONSE_MESSAGE";
  ...
  case Event::STA_Steering_Probe_Req: {
    ...
    auto msg = static_cast<
        bwl::sACTION_APMANAGER_STEERING_EVENT_PROBE_REQ_NOTIFICATION *>(data);

    LOG(INFO) << "CLIENT_SOFTBLOCK_NOTIFICATION for client mac "
              << msg->params.client_mac;

    // Build the response message
    auto notification = message_com::create_vs_message<
        beerocks_message::
            cACTION_APMANAGER_STEERING_EVENT_PROBE_REQ_NOTIFICATION>(cmdu_tx);
    if (notification == nullptr) {
      LOG(ERROR) << "Failed building "
  ...
  case Event::STA_Steering_Auth_Fail: {
    ...
    auto msg = static_cast<
        bwl::sACTION_APMANAGER_STEERING_EVENT_AUTH_FAIL_NOTIFICATION *>(data);
    ...
    auto notification = message_com::create_vs_message<
        beerocks_message::
            cACTION_APMANAGER_STEERING_EVENT_AUTH_FAIL_NOTIFICATION>(cmdu_tx);
    if (notification == nullptr) {
      LOG(ERROR) << "Failed building "
  ...
  case Event::DFS_CAC_Started: {
    ...
    auto msg = static_cast<
        bwl::sACTION_APMANAGER_HOSTAP_DFS_CAC_STARTED_NOTIFICATION *>(data);
    ...
    auto response = message_com::create_vs_message<
        beerocks_message::
            cACTION_APMANAGER_HOSTAP_DFS_CAC_STARTED_NOTIFICATION>(cmdu_tx);
    if (!response) {
      LOG(ERROR) << "Failed building "
  ...
  case Event::DFS_CAC_Completed: {
    ...
    auto msg = static_cast<
        bwl::sACTION_APMANAGER_HOSTAP_DFS_CAC_COMPLETED_NOTIFICATION *>(data);
    LOG(INFO) << "DFS_EVENT_CAC_COMPLETED success = "
              << int(msg->params.success);

    auto response = message_com::create_vs_message<
        beerocks_message::
            cACTION_APMANAGER_HOSTAP_DFS_CAC_COMPLETED_NOTIFICATION>(cmdu_tx);
    if (response == nullptr) {
      LOG(ERROR) << "Failed building "
                    "ACTION_APMANAGER_HOSTAP_DFS_CAC_COMPLETED_NOTIFICATION "
                    "message!";
      break;
    }
  ...
  case Event::DFS_NOP_Finished: {
    ...
    auto msg = static_cast<
        bwl::sACTION_APMANAGER_HOSTAP_DFS_CHANNEL_AVAILABLE_NOTIFICATION *>(
        data);
    LOG(INFO) << "DFS_EVENT_NOP_FINISHED "
              ...
              << " vht_center_frequency = "
              << int(msg->params.vht_center_frequency);
    ...
    if (response == nullptr) {
      LOG(ERROR) << "Failed building "
                    "ACTION_APMANAGER_HOSTAP_DFS_CHANNEL_AVAILABLE_"
  ...
  case Event::AP_Disabled: {
    ...
    if (msg->vap_id == beerocks::IFACE_RADIO_ID) {
      ...
      auto response = message_com::create_vs_message<
          beerocks_message::cACTION_APMANAGER_HOSTAP_AP_DISABLED_NOTIFICATION>(
          cmdu_tx);
      if (response == nullptr) {
        LOG(ERROR)
            << "Failed building "
               "cACTION_APMANAGER_HOSTAP_AP_DISABLED_NOTIFICATION message!";
        break;
      }
  ...
  case Event::MGMT_Frame: {
    if (!data) {
      ...
      // That's indeed an error, but no reason to terminate the AP Manager in
      // this case. Return "true" to ignore the event and continue operating.
      return true;
    }
    ...
    if (mgmt_frame->type ==
        bwl::eManagementFrameType::RADIO_MEASUREMENT_REPORT) {
      LOG(DEBUG) << "Received RADIO_MEASUREMENT_REPORT from "
                 << mgmt_frame->mac;
      ...
      /* Skip first 3 bytes(action frame category + action frame code + dialog
       * token) */
      idx += 3;

      /* Since element_id and length are not included in the element's length, 2
       * bytes are subtracted */
      size_t min_measurement_length =
          wfa_map::cMeasurementReportElement::get_initial_size() - 2;

      /* Return true in case of an invalid management frame, no reason to
       * terminate the AP Manager */
      while (idx < mgmt_frame->data.size()) {
        sBeaconMetricsResponse::sBeaconReport beacon_report = {};
        ...
        if (element_id !=
            wfa_map::cMeasurementReportElement::ID_MEASUREMENT_REPORT) {
          LOG(ERROR) << "Incorrect element ID: " << element_id;
          return true;
        }
        ...
        if (element_length < min_measurement_length) {
          LOG(ERROR) << "Invalid measurement element length: "
                     << element_length;
          return true;
        }

        /* 802.11-2020 9.4.2.21.7 Beacon report->Optional_Subelements are
         * ignored */
        beacon_report.length = min_measurement_length;
        ...
        if (measurement_type !=
            wfa_map::cMeasurementReportElement::TYPE_BEACON) {
          LOG(ERROR) << "Incorrect measurement type: " << measurement_type;
          return true;
        }
    ...
    // Convert the BWL type to a tunnelled message type
    wfa_map::tlvTunnelledProtocolType::eTunnelledProtocolType
        tunnelled_proto_type;
    switch (mgmt_frame->type) {
    case bwl::eManagementFrameType::ASSOCIATION_REQUEST: {
      tunnelled_proto_type = wfa_map::tlvTunnelledProtocolType::
          eTunnelledProtocolType::ASSOCIATION_REQUEST;
    } break;
    case bwl::eManagementFrameType::REASSOCIATION_REQUEST: {
      tunnelled_proto_type = wfa_map::tlvTunnelledProtocolType::
          eTunnelledProtocolType::REASSOCIATION_REQUEST;
    } break;
    case bwl::eManagementFrameType::BTM_QUERY: {
      tunnelled_proto_type =
          wfa_map::tlvTunnelledProtocolType::eTunnelledProtocolType::BTM_QUERY;
    ...
    case bwl::eManagementFrameType::WNM_REQUEST: {
      tunnelled_proto_type = wfa_map::tlvTunnelledProtocolType::
          eTunnelledProtocolType::WNM_REQUEST;
    } break;
    case bwl::eManagementFrameType::ANQP_REQUEST: {
      tunnelled_proto_type = wfa_map::tlvTunnelledProtocolType::
          eTunnelledProtocolType::ANQP_REQUEST;
    } break;
    default: {
      LOG(DEBUG) << "Unsupported 802.11 management frame: " << std::hex
                 << int(mgmt_frame->type);

      // Not supporting a specific frame is not really an error, so just stop
      // processing
      return true;
    }
    }
    ...
    // Create a tunnelled message
    auto cmdu_tx_header =
        cmdu_tx.create(0, ieee1905_1::eMessageType::TUNNELLED_MESSAGE);
    if (!cmdu_tx_header) {
      LOG(ERROR) << "cmdu creation of type TUNNELLED_MESSAGE failed!";
      return false;
    }
    ...
    if (mgmt_frame->type == bwl::eManagementFrameType::BTM_QUERY) {
      // pwhm sets the following flags in the Request Mode Field of the 802.11
      // BTM Request : M_SWL_IEEE802_BTM_REQ_MODE_PREF_LIST_INCL |
      ...
      if (it == vap_unordered_map.end()) {
        // AP does not have the requested vap, probably will be handled on the
        // other AP
        LOG(DEBUG) << "AP does not have the requested vap, probably will be "
                      "handled on the "
                      "other AP";
        break;
      }
  ...
  case Event::ACL_DENY: {
    ...
    sta_mac_tlv->sta_mac() = sta_conn_fail->sta_mac;
    // add status code
    auto profile2_status_code_tlv =
        cmdu_tx.addClass<wfa_map::tlvProfile2StatusCode>();
    if (!profile2_status_code_tlv) {
      LOG(ERROR) << "addClass wfa_map::tlvProfile2StatusCode!";
      return false;
    }
    profile2_status_code_tlv->status_code() = sta_conn_fail->status;
    auto profile2_reason_code_tlv =
        cmdu_tx.addClass<wfa_map::tlvProfile2ReasonCode>();
    if (!profile2_reason_code_tlv) {
      LOG(ERROR) << "addClass wfa_map::tlvProfile2ReasonCode!";
      return false;
    }
    profile2_reason_code_tlv->reason_code() =
        static_cast<wfa_map::tlvProfile2ReasonCode::eReasonCode>(
            sta_conn_fail->reason);
    // Send the mismatched message
    send_cmdu(cmdu_tx);
  } break;
  case Event::DPP_PRESENCE_ANNOUNCEMENT: {
    LOG(DEBUG) << "DPP Presence Announcement";
    auto dpp_presence =
        static_cast<bwl::sACTION_APMANAGER_DPP_PRESENCE_ANNOUNCEMENT *>(data);
  ...
  case Event::DPP_AUTHENTICATION_RESPONSE: {
    ...
    encap_1905_dpp_tlv->frame_flags().dpp_frame_indicator = false;
    encap_1905_dpp_tlv->frame_flags().enrollee_mac_address_present = true;
    encap_1905_dpp_tlv->set_dest_sta_mac(
        dpp_authentication_response->enrollee_mac);
    send_cmdu(cmdu_tx);
  } break;
  case Event::DPP_CONFIGURATION_REQUEST: {
    LOG(DEBUG) << "DPP Configuration Request";
    ...
    encap_1905_dpp_tlv->frame_type() =
        wfa_map::tlv1905EncapDpp::eFrameType::GAS_FRAME;
    encap_1905_dpp_tlv->frame_flags().dpp_frame_indicator = true;
    encap_1905_dpp_tlv->frame_flags().enrollee_mac_address_present = true;
    encap_1905_dpp_tlv->set_dest_sta_mac(
        dpp_configuration_request->enrollee_mac);
    send_cmdu(cmdu_tx);
  } break;
  // Unhandled events
  default:
...
void ApManager::handle_hostapd_attached() {
  ...
  if (acs_enabled) {
    ...
    while (!ap_wlan_hal->read_acs_report()) {
      read_acs_attempt++;
      if (read_acs_attempt >= READ_ACS_ATTEMPT_MAX) {
        LOG(ERROR) << "retrieving ACS report fails "
                   << int(READ_ACS_ATTEMPT_MAX) << " times - stop ApManager";
        m_state = eApManagerState::TERMINATED;
        break;
      }
  ...
  auto notification = message_com::create_vs_message<
      beerocks_message::cACTION_APMANAGER_JOINED_NOTIFICATION>(cmdu_tx);
  ...
  notification->params().iface_type = uint8_t(ap_wlan_hal->get_iface_type());
  notification->params().iface_mac =
      tlvf::mac_from_string(ap_wlan_hal->get_radio_mac());
  notification->params().ant_num = ap_wlan_hal->get_radio_info().ant_num;
  notification->params().tx_power = ap_wlan_hal->get_radio_info().tx_power;

  notification->params().frequency_band =
      ap_wlan_hal->get_radio_info().frequency_band;
  notification->params().max_bandwidth =
      ap_wlan_hal->get_radio_info().max_bandwidth;
  notification->params().ht_supported =
      ap_wlan_hal->get_radio_info().ht_supported;
  notification->params().ht_capability =
      ap_wlan_hal->get_radio_info().ht_capability;
  std::copy_n(ap_wlan_hal->get_radio_info().ht_mcs_set.data(),
              beerocks::message::HT_MCS_SET_SIZE,
              notification->params().ht_mcs_set);
  notification->params().vht_supported =
      ap_wlan_hal->get_radio_info().vht_supported;
  notification->params().vht_capability =
      ap_wlan_hal->get_radio_info().vht_capability;
  std::copy_n(ap_wlan_hal->get_radio_info().vht_mcs_set.data(),
              beerocks::message::VHT_MCS_SET_SIZE,
              notification->params().vht_mcs_set);
  notification->params().he_supported =
      ap_wlan_hal->get_radio_info().he_supported;
  notification->params().he_capability =
      ap_wlan_hal->get_radio_info().he_capability;
  notification->params().wifi6_capability =
      ap_wlan_hal->get_radio_info().wifi6_capability;
  std::copy_n(ap_wlan_hal->get_radio_info().he_mcs_set.data(),
              beerocks::message::HE_MCS_SET_SIZE,
              notification->params().he_mcs_set);
  notification->params().eht_supported =
      ap_wlan_hal->get_radio_info().eht_supported;
  ...
  notification->params().bsta_modes_support = bsta_modes_support;
  LOG(DEBUG) << "[WiFi7]AP MLO Modes Support "
             << notification->params().ap_modes_support
             << "\n | BSTA Mode Support "
             << notification->params().bsta_modes_support;

  notification->params().max_num_mlds =
      ap_wlan_hal->get_radio_info().max_num_mlds;
  notification->params().ap_maximum_links =
      ap_wlan_hal->get_radio_info().ap_maximum_links;
  notification->params().bsta_maximum_links =
      ap_wlan_hal->get_radio_info().bsta_maximum_links;

  notification->params().zwdfs = m_ap_support_zwdfs;

  string_utils::copy_string(
      notification->params().chipset_vendor,
      ap_wlan_hal->get_radio_info().chipset_vendor.c_str(),
      beerocks::message::CHIPSET_VENDOR_LENGTH);

  notification->params().hybrid_mode_supported =
      ap_wlan_hal->hybrid_mode_supported();

  notification->radio_max_bss() =
      ap_wlan_hal->get_radio_info().radio_max_bss_supported;
  notification->radio_rsn_override_support() =
      ap_wlan_hal->get_radio_info().rsn_override_support;
  ...
  LOG(INFO) << " tx_power = " << ap_wlan_hal->get_radio_info().tx_power;
  LOG(INFO) << " current channel = " << ap_wlan_hal->get_radio_info().channel;
  LOG(INFO) << " vht_center_frequency = "
            << ap_wlan_hal->get_radio_info().vht_center_freq;
  LOG(INFO) << " current bw = " << ap_wlan_hal->get_radio_info().bandwidth;
  LOG(INFO) << " frequency_band = "
            << beerocks::utils::convert_frequency_type_to_string(
                   ap_wlan_hal->get_radio_info().frequency_band);
  LOG(INFO) << " max_bandwidth = "
            << ap_wlan_hal->get_radio_info().max_bandwidth;
  LOG(INFO) << " ht_supported = " << ap_wlan_hal->get_radio_info().ht_supported;
  LOG(INFO) << " ht_capability = " << std::hex
            << ap_wlan_hal->get_radio_info().ht_capability;
  LOG(INFO) << " vht_supported = "
            << ap_wlan_hal->get_radio_info().vht_supported;
  LOG(INFO) << " vht_capability = " << std::hex
            << ap_wlan_hal->get_radio_info().vht_capability;
  LOG(INFO) << " he_supported = " << ap_wlan_hal->get_radio_info().he_supported;
  LOG(INFO) << " he_capability = " << std::hex
            << ap_wlan_hal->get_radio_info().he_capability;
  LOG(INFO) << " wifi6_capability = " << std::hex
            << ap_wlan_hal->get_radio_info().wifi6_capability;
  LOG(INFO) << " eht_supported = "
            << ap_wlan_hal->get_radio_info().eht_supported;
  LOG(INFO) << " max_num_mld = " << ap_wlan_hal->get_radio_info().max_num_mlds;
  LOG(INFO) << " ap_maximum_links = "
            << ap_wlan_hal->get_radio_info().ap_maximum_links;
  LOG(INFO) << " bsta_maximum_links = "
            << ap_wlan_hal->get_radio_info().bsta_maximum_links;
  LOG(INFO) << " zwdfs = " << m_ap_support_zwdfs;
  LOG(INFO) << " radio_max_bss = "
            << ap_wlan_hal->get_radio_info().radio_max_bss_supported;
  LOG(INFO) << " chipset_vendor = "
            << ap_wlan_hal->get_radio_info().chipset_vendor;
...
void ApManager::send_heartbeat() {
  // LOG(DEBUG) << "sending HEARTBEAT notification";
  auto request = message_com::create_vs_message<
      beerocks_message::cACTION_APMANAGER_HEARTBEAT_NOTIFICATION>(cmdu_tx);

  if (request == nullptr) {
    LOG(ERROR)
        << "Failed building cACTION_APMANAGER_HEARTBEAT_NOTIFICATION message!";
    return;
  }
...
bool ApManager::handle_ap_enabled(int vap_id) {
  ...
  LOG(INFO) << "vap_id = " << int(vap_id) << ", bssid = " << vap_info.mac
            << ", ssid = " << vap_info.ssid
            << ", fronthaul = " << vap_info.fronthaul
            << ", backhaul = " << vap_info.backhaul
            << ", apmld mac = " << vap_info.ap_mld_mac
  ...
  auto notification = message_com::create_vs_message<
      beerocks_message::cACTION_APMANAGER_HOSTAP_AP_ENABLED_NOTIFICATION>(
      cmdu_tx);
  if (!notification) {
    LOG(ERROR) << "Failed building "
                  "cACTION_APMANAGER_HOSTAP_AP_ENABLED_NOTIFICATION message!";
    return false;
  }
  ...
  string_utils::copy_string(notification->vap_info().iface_name,
                            vap_info.bss.c_str(),
                            beerocks::message::IFACE_NAME_LENGTH);
  notification->vap_info().mac = tlvf::mac_from_string(vap_info.mac);
  string_utils::copy_string(notification->vap_info().ssid,
                            vap_info.ssid.c_str(),
                            beerocks::message::WIFI_SSID_MAX_LENGTH);
  ...
  notification->vap_info().ap_mld_mac =
      tlvf::mac_from_string(vap_info.ap_mld_mac);
...
void ApManager::send_steering_return_status(
    beerocks_message::eActionOp_APMANAGER ActionOp, int32_t status) {
  switch (ActionOp) {
  ...
  case beerocks_message::ACTION_APMANAGER_STEERING_CLIENT_SET_RESPONSE: {
    auto response = message_com::create_vs_message<
        beerocks_message::cACTION_APMANAGER_STEERING_CLIENT_SET_RESPONSE>(
        cmdu_tx);
    if (response == nullptr) {
      LOG(ERROR) << "Failed building message!";
      break;
...
void ApManager::remove_client_from_disallowed_list(const sMacAddr &mac,
                                                   const sMacAddr &bssid) {
  auto it =
      std::find_if(m_disallowed_clients.begin(), m_disallowed_clients.end(),
...
void ApManager::allow_expired_clients() {
  ...
  for (auto it = m_disallowed_clients.begin();
       it != m_disallowed_clients.end();) {
    if (std::chrono::steady_clock::now() > it->timeout) {
      LOG(DEBUG) << "CLIENT_ALLOW: mac = " << it->mac
                 << ", bssid = " << it->bssid;
...
bool ApManager::is_operational() const {
  return m_state == eApManagerState::OPERATIONAL;
}

bool ApManager::zwdfs_ap() const {
  if (m_state != eApManagerState::OPERATIONAL) {
    LOG(WARNING)
        << "Requested ZWDFS support status, but AP is not attached to BWL";
    return true;
  }
...
bool ApManager::add_bss(std::string &ifname,
                        son::wireless_utils::sBssInfoConf &bss_conf,
  ...
  beerocks::EventLoop::EventHandlers ext_events_handlers{
      ...
      .on_disconnect =
          [&](int fd, EventLoop &loop) {
            ...
            auto it = std::find(m_ap_hal_ext_events.begin(),
                                m_ap_hal_ext_events.end(), fd);
            if (it != m_ap_hal_ext_events.end()) {
              *it = beerocks::net::FileDescriptor::invalid_descriptor;
            }
            auto if_it = std::find_if(
                m_dynamic_bss_ext_events_fds.begin(),
                m_dynamic_bss_ext_events_fds.end(),
...
void ApManager::handle_trigger_channel_switch_announcement_request(
  ...
  // TODO: Implement (unicast) CSA in BWL and call it here based on contents of
  // the CSA TLV! PPM-2194

  if (!cmdu_tx.create(
          0, ieee1905_1::eMessageType::
                 TRIGGER_CHANNEL_SWITCH_ANNOUNCEMENT_RESPONSE_MESSAGE)) {
    LOG(ERROR)
        << "cmdu creation of type "
           "TRIGGER_CHANNEL_SWITCH_ANNOUNCEMENT_RESPONSE_MESSAGE failed!";
    return;
  }

  auto client_info_tlv_tx = cmdu_tx.addClass<wfa_map::tlvClientInfo>();
  if (!client_info_tlv_tx) {
    LOG(ERROR) << "Failed to add Client Info TLV to the Trigger Channel Switch "
  ...
  auto csa_tlv_tx =
      cmdu_tx.addClass<wfa_map::TriggerChannelSwitchAnnouncement>();
  if (!csa_tlv_tx) {
    LOG(ERROR) << "Failed to add Trigger Channel Switch Announcement TLV to "
                  "the response!";
    return;
  }
  ...
  LOG(DEBUG) << __func__
             << " - NOT IMPLEMENTED! Not sending response to controller.";
  // TODO -- send_cmdu once BWL is implemented and error checking can be done.
}

void ApManager::handle_vbss_security_request(
    ieee1905_1::CmduMessageRx &cmdu_rx) {
  ...
  if (!cmdu_tx.create(
    ...
    LOG(ERROR) << "cmdu creation of type "
                  "CLIENT_SECURITY_CONTEXT_REQUEST_MESSAGE failed!";
    return;
  }
  ...
  auto client_security_context_tlv =
      cmdu_tx.addClass<wfa_map::ClientSecurityContext>();
  ...
  client_security_context_tlv->set_ptk(pairwise_key.key.data(),
                                       pairwise_key.key.size());
  ...
  client_security_context_tlv->set_gtk(group_key.key.data(),
                                       group_key.key.size());
  if (group_key.key_seq.size() > sizeof(uint64_t)) {
    LOG(ERROR) << "Key sequence bigger than allowed size!";
    return;
  }
  client_security_context_tlv->set_group_tx_packet_num(
      group_key.key_seq.data(), group_key.key_seq.size());
...
void ApManager::csa_notification_timer_elapsed(
  ...
  auto spatial_reuse_report = message_com::create_vs_message<
      beerocks_message::
          cACTION_APMANAGER_HOSTAP_SPATIAL_REUSE_REPORT_NOTIFICATION>(cmdu_tx);
  ...
  // Sends the cACTION_APMANAGER_HOSTAP_CSA_NOTIFICATION event to the
  // son_slave_thread to trigger the CSA notification
  auto notification = message_com::create_vs_message<
      beerocks_message::cACTION_APMANAGER_HOSTAP_CSA_NOTIFICATION>(cmdu_tx);
  if (!notification) {
    LOG(ERROR) << "Failed building message!";
...
void ApManager::start_csa_notification_timer(
  ...
  // Start a timer for approximately 5 seconds (timer should be configurable
  // based on the CSA count). When the timer elapses, the
  // "csa_notification_timer_elapsed" function will be called
  csa_notification_timer = m_timer_manager->add_timer(
      "csa_notification", 5s, 0s,
      [this, request = std::move(request)](int, auto &) mutable {
