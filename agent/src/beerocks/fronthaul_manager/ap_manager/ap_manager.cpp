...
bool ApManager::hash_match_found = false;
...
bool ApManager::store_enrollee_auth_request(const sMacAddr &sta_mac,
                                            ...
                                            const wfa_map::tlv1905EncapDpp::sFlags &flags)
{
    ...
    entry.hash       = hash;
    LOG(INFO) << "Stored Auth Request and Hash for " << tlvf::mac_to_string(sta_mac);
    return true;
}
...
void ApManager::handle_proxied_encap_dpp_message(ieee1905_1::CmduMessageRx &cmdu_rx)
{
    ...
    sMacAddr dest_sta_mac     = *encap_1905_dpp_tlv->dest_sta_mac();
    ...
    if (!ApManager::store_enrollee_auth_request(dest_sta_mac, frame_vec, hash_vec,
                                                dpp_inidicator_flags)) {
        LOG(ERROR) << "Failed to store DPP Auth Request for " << tlvf::mac_to_string(dest_sta_mac);
        return;
    }
    if (!ApManager::hash_match_found) {
        ...
        for (const auto &vap_pair : radio_info.available_vaps) {
            ...
            std::string vap_iface_name = vap.bss; // This is the interface name/alias
            // Now iface_name is a VAP on the current channel
            LOG(INFO) << "VAP interface: " << vap_iface_name << " is on channel: " << channel
                      << " with MAC: " << vap.mac;

            if (!ApManager::send_dpp_auth_to_enrollee(vap_iface_name, channel, dest_sta_mac,
                                                      auth_frame, auth_frame_len,
                                                      dpp_inidicator_flags)) {
...
son::ApManager::build_dpp_auth_response_frame(const std::vector<unsigned char> &input)
{
    std::vector<uint8_t> frame;
    frame.reserve(8 + input.size()); // Header + attributes

    frame.push_back(dpp_frame::CATEGORY_PUBLIC_ACTION);
    frame.push_back(dpp_frame::ACTION_VENDOR_SPECIFIC);
    frame.insert(frame.end(), std::begin(dpp_frame::OUI_WFA), std::end(dpp_frame::OUI_WFA));
    frame.push_back(dpp_frame::OUI_TYPE_DPP);
    frame.push_back(dpp_frame::CRYPTO_SUITE);
    frame.push_back(dpp_frame::SUBTYPE_AUTHENTICATION_RESP);
...
bool ApManager::hal_event_handler(bwl::base_wlan_hal::hal_event_ptr_t event_ptr)
{
    ...
    case Event::DPP_PRESENCE_ANNOUNCEMENT: {
        ...
        for (const auto &entry : m_enrollee_auth_map) {
            ...
            if (info.hash.size() == binary_hash.size() &&
                ...
                if (!ApManager::send_dpp_auth_to_enrollee(dpp_presence->vap_interface, channel,
                                                          info.mac, info.auth_frame.data(),
                                                          info.auth_frame.size(), info.flags)) {
    ...
    case Event::DPP_AUTHENTICATION_RESPONSE: {
        ...
        auto full_frame = build_dpp_auth_response_frame(dpp_attributes);
