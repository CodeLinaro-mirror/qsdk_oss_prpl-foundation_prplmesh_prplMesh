...
bool Monitor::update_sta_stats(const std::chrono::steady_clock::time_point &timeout)
{
    ...
    std::unordered_map<int8_t, std::vector<StaStatsUpdate>> stations_by_vap;
    ...
    for (auto it = mon_db.sta_begin(); it != mon_db.sta_end(); ++it) {
        ...
        if (!vap_node) {
            LOG(ERROR) << "Station " << sta_mac << " has no VAP node for id "
                       << int(sta_node->get_vap_id());
            continue;
        }
        ...
        stations_by_vap[sta_node->get_vap_id()].push_back({std::move(sta_mac), sta_node});
    }

    const auto is_read_unicast = mon_db.get_clients_unicast_measurements();
    for (auto &vap_stations : stations_by_vap) {
        auto vap_node = mon_db.vap_get_by_id(vap_stations.first);
        if (!vap_node) {
            LOG(ERROR) << "VAP node disappeared for id " << int(vap_stations.first);
            continue;
        }
        ...
        std::vector<std::string> sta_macs;
        std::unordered_map<std::string, bwl::SStaStats> vap_sta_stats;
        sta_macs.reserve(vap_stations.second.size());
        for (auto &station : vap_stations.second) {
        ...
        for (auto &station : vap_stations.second) {
            ...
            if (stats_it == vap_sta_stats.end()) {
                LOG(ERROR) << "Failed updating STA (" << station.mac << ") statistics!";
                continue;
            }
