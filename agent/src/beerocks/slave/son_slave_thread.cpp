        ...
        notification_in)
{
    ...
    if (notification_in->is_mlo()) {
        ...
        if (client_it != radio->associated_clients.end() &&
            client_it->second.wds_iface_name.empty() && notification_in->multi_ap_profile() != 0) {
            ...
            if (wds_ifaces.empty()) {
                LOG(DEBUG) << "No WDS iface recovered from BH STA association sta_mac="
                           << client_mac << " bssid=" << bssid << " bss_iface=" << bss_iface_name;
                ...
                for (const auto &wds_iface : wds_ifaces) {
                    LOG(INFO) << "Recovered WDS iface from BH STA association sta_mac="
                              << client_mac << " bssid=" << bssid << " wds_iface=" << wds_iface;
                    task_pool_try_send_event(eTaskType::TRAFFIC_SEPARATION,
                                        ...
                                        const sMacAddr &bssid, const std::string &wds_iface_name)
{
    ...
    if (!client.wds_iface_name.empty()) {
        notify_clear_wds_iface(client.wds_iface_name);
    }

    client.wds_iface_name = wds_iface_name;
    notify_new_wds_iface(client.wds_iface_name);
                                              ...
                                              const sMacAddr &primary_bssid)
{
    ...
    // Keep the primary BSSID to correlate a following parent disconnect or reconnect.
                                                  ...
                                                  const std::string &fronthaul_iface)
{
    ...
    LOG(DEBUG) << "Deferring MLO parent disconnect for " << client_mac
               << " while validating primary WDS state";
    return true;
}
                                                  ...
                                                  std::shared_ptr<beerocks_header> beerocks_header)
{
    ...
    case beerocks_message::ACTION_APMANAGER_CLIENT_ASSOCIATED_NOTIFICATION: {
        ...
        const auto mld_it    = db->associated_sta_mlds.find(client_mac);
        bool same_mlo_parent = false;
        if (mld_it != db->associated_sta_mlds.end()) {
            const auto &mld_info = mld_it->second;
            // WHM can briefly report the active MLO parent without APMLDMacAddress.
            same_mlo_parent = notification_in->is_mlo() ? bssid == mld_info.mld_config.ap_mld_mac
                                                        : bssid == mld_info.primary_bssid;
            if (same_mlo_parent && !mld_info.wds_iface_name.empty() &&
        ...
        const bool transient_legacy_mlo_reconnect =
            !notification_in->is_mlo() && same_mlo_parent &&
            pending_disconnect_it != m_pending_mlo_disconnections.end();
        ...
        if (transient_legacy_mlo_reconnect) {
            auto &mld_info = mld_it->second;
            if (!mld_info.wds_iface_name.empty() &&
                !network_utils::linux_iface_is_up_and_running(mld_info.wds_iface_name)) {
                clear_mlo_client_wds_iface(mld_info, client_mac, mld_info.primary_bssid);
            }
            LOG(INFO) << "Ignoring transient legacy-looking reconnect for MLO client " << client_mac
                      << ", bssid=" << bssid;
            break;
        }
...
bool slave_thread::add_eht_operations_tlv(ieee1905_1::CmduMessageTx &cmdu_tx)
{
