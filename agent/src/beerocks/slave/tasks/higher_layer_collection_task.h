...
public:
    ...
    /** @brief Callback type for retrieving the current FriendlyName. */
    using get_friendly_name_f = std::function<std::string()>;
     ...
     * @param get_friendly_name Optional callback to override FriendlyName retrieval.
     */
    HigherLayerCollectionTask(send_cmdu_to_mac_f send_cmdu_to_mac, get_bridge_mac_f get_bridge_mac,
                              ...
                              get_friendly_name_f get_friendly_name = {});
...
private:
    ...
    get_friendly_name_f m_get_friendly_name;                              ///< FriendlyName getter.
