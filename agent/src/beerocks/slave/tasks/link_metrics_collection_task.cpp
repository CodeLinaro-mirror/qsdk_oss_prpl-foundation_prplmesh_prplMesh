    ...
    ieee1905_1::CmduMessageRx &cmdu_rx, const sMacAddr &src_mac)
{
    ...
    // TODO: Even if MLD or WiFi6 STA found Empty Metrics send (PPM-3823)
    auto db = AgentDB::get();
    ...
    if (it != db->associated_sta_mlds.end()) {
        LOG(DEBUG) << "MLD STA Found MAC: " << mac->sta_mac();
    ...
    // Error condition Not in MLD Not in Legacy
    ...
    ieee1905_1::CmduMessageRx &cmdu_rx, const sMacAddr &src_mac, const sMacAddr &agent_mac)
{
    ...
    const uint8_t oper_class =
        unassociated_sta_link_metrics_query_tlv->operating_class_of_channel_list();
                                                           ...
                                                           const sMacAddr &src_mac)
{
    ...
    if (pending_responses == 0) {
        LOG(DEBUG) << "pending AP_Metrics responses is already zero for MID : " << std::hex
                   << mid_index;
        return;
    }
    ...
    std::map<sLinkInterface, std::set<sLinkNeighbor>> &neighbor_links_map)
{
    ...
    // On a gateway the controller sits behind a veth, missed by the interfaces above.
    const auto &controller_al_mac = db->controller_info.bridge_mac;
    if (controller_al_mac == net::network_utils::ZERO_MAC ||
