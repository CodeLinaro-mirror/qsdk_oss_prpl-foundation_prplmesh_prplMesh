                                                                ...
                                                                const sMacAddr &ruid)
{
    ...
    auto process_missing_mld_links = [&](const sMacAddr &bsta_ruid) {
        for (const auto &curr_bsta_mld : curr_bsta_mld_infos) {
            const auto &curr_ruid = curr_bsta_mld.first;

            // Check if current MLD exists in new requests
            auto it = bsta_mld_requests_infos.find(curr_ruid);

            // If not found, add with DISABLED_MLDUNIT
            if (it == bsta_mld_requests_infos.end()) {
                send_bsta_mld_configuration(
                    curr_ruid, DISABLED_MLDUNIT,
