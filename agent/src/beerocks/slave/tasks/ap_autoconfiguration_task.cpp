...
bool ApAutoConfigurationTask::handle_wsc_m2_tlv(
    ...
    const std::vector<WSC::m2> &m2_list, std::vector<sBssInfo> &infos,
    std::unordered_set<std::string> &misconfigured_ssids)
{
    auto db    = AgentDB::get();
    auto radio = db->radio(radio_iface);
...
bool ApAutoConfigurationTask::handle_wsc_m8_tlv(const std::string &radio_iface,
                                                std::shared_ptr<WSC::m8> m8, sBStaInfo &info)
{
    auto db    = AgentDB::get();
    auto radio = db->radio(radio_iface);
    uint8_t authkey[32];
...
bool ApAutoConfigurationTask::handle_agent_ap_mld_configuration_tlv(
    ieee1905_1::CmduMessageRx &cmdu_rx, std::vector<sBssInfo> &infos,
    const std::string &radio_iface)
{
    auto db(AgentDB::get());
    bool ret_code(true);
...
bool ApAutoConfigurationTask::handle_rsn_parameters_configuration_tlv(
    ieee1905_1::CmduMessageRx &cmdu_rx, std::vector<sBssInfo> &infos, const sMacAddr &ruid,
                                                       ...
                                                       std::vector<sBssInfo> &infos)
{
    ...
    const auto find_by_similarity = [&db](const AgentDB::sRadio::sFront::sBssid &bss) {
        return [&db, &bss](const sBssInfo &info) {
    ...
    const auto bss_needs_reconfiguration = [](const sBssInfo &info,
    ...
    const auto bss_pending_teardown = [](const sBssInfo &info) {
    ...
    std::vector<sBssInfo> infos_copy = infos;
    std::vector<sBssInfo> final_infos;
...
bool ApAutoConfigurationTask::send_ap_bss_configuration_message(const std::string &radio_iface,
                                                                const std::vector<sBssInfo> &infos)
{
    auto request = message_com::create_vs_message<
