...
void TrafficSeparationTask::schedule_apply(eApplyMode mode)
{
    ...
    if (m_next_run.time_since_epoch().count() == 0 || m_next_run < now) {
        m_next_run = std::max(m_next_run, due);
    }
}
...
{
    LOG(TRACE) << "TrafficSeparationTask::ensure_transport_primary_vlan | primary_vid = "
               << primary_vid;
    ...
    if (!m_btl_ctx.m_broker_client) {
        LOG(WARNING) << "TrafficSeparationTask: broker_client is null, skip";
        return false;
    }
...
{
    LOG(TRACE) << "TrafficSeparationTask::reset_transport_monitoring_on_bridge";
    if (!m_btl_ctx.m_broker_client) {
        LOG(WARNING) << "TrafficSeparationTask: broker_client is null, skip";
        return false;
    }
