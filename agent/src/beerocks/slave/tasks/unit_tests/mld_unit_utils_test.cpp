...
TEST(MldUnitUtilsTest, RejectsPreconfiguredUnitOwnedByDifferentSsid)
{
    const auto selection =
        select_ap_mld_unit("backhaul", {{}}, 4, {{"home", 2}}, {{"backhaul", 2}});
...
// cppcheck-suppress syntaxError
TEST(MldUnitUtilsTest, IndependentSameSsidGroupsKeepTheirUnitsInEitherOrder)
{
    const sMacAddr radio0{{0x02, 0, 0, 0, 0, 1}};
    const sMacAddr radio1{{0x02, 0, 0, 0, 0, 2}};
    const std::vector<sMldUnitAssignment> local{{"home", 0, radio0}, {"home", 1, radio1}};
    for (bool reverse : {false, true}) {
        std::vector<sMldUnitAssignment> assigned;
        for (int index : {int(reverse), int(!reverse)}) {
            const auto &group    = local[index];
            const auto selection = select_ap_mld_unit("home", {group.ruid}, 4, assigned, local);
            EXPECT_FALSE(selection.conflicting_preconfigured_units);
            EXPECT_FALSE(selection.preconfigured_unit_in_use);
            ASSERT_EQ(selection.mld_unit, group.mld_unit);
            assigned.emplace_back("home", selection.mld_unit);
        }
    }
    const MldRequests requests{
        {"wlan0", {{"home", {select_ap_mld_unit("home", {radio0}, 4, {}, local).mld_unit, 0}}}},
        {"wlan1", {{"home", {select_ap_mld_unit("home", {radio1}, 4, {}, local).mld_unit, 0}}}}};
    ...
    populate_mld_id_in_bss_infos("wlan1", requests, configs);
    EXPECT_EQ(configs[0].mld_id, 1);
}

// cppcheck-suppress syntaxError
TEST(MldUnitUtilsTest, UnrelatedSameSsidGroupDoesNotConflictWithRequestedMultiLinkGroup)
{
    const sMacAddr radio0{{0x02, 0, 0, 0, 0, 1}};
    const sMacAddr radio1{{0x02, 0, 0, 0, 0, 2}};
    const sMacAddr radio2{{0x02, 0, 0, 0, 0, 3}};
    const auto selection =
        select_ap_mld_unit("home", {radio0, radio1}, 4, {},
                           {{"home", 2, radio0}, {"home", 2, radio1}, {"home", 1, radio2}});
    EXPECT_FALSE(selection.conflicting_preconfigured_units);
    EXPECT_EQ(selection.mld_unit, 2);
}

// cppcheck-suppress syntaxError
TEST(MldUnitUtilsTest, UnitOnUnaffiliatedRadioStaysReservedEvenWithSameSsid)
{
    const sMacAddr radio0{{0x02, 0, 0, 0, 0, 1}};
    const sMacAddr radio1{{0x02, 0, 0, 0, 0, 2}};
    EXPECT_EQ(select_ap_mld_unit("home", {radio0}, 4, {}, {{"home", 0, radio1}}).mld_unit, 1);
    const auto selection =
        select_ap_mld_unit("home", {radio0}, 4, {}, {{"home", 0, radio0}, {"home", 0, radio1}});
    EXPECT_TRUE(selection.preconfigured_unit_in_use);
    EXPECT_EQ(selection.mld_unit, 1);
}

// cppcheck-suppress syntaxError
TEST(MldUnitUtilsTest, AlreadyAssignedUnitCannotBeReusedByAnotherSameSsidGroup)
{
    const auto selection = select_ap_mld_unit("home", {{}}, 4, {{"home", 2}}, {{"home", 2}});
    EXPECT_TRUE(selection.preconfigured_unit_in_use);
    EXPECT_EQ(selection.mld_unit, 0);
