    ...
    const std::shared_ptr<sScanRequest> request)
{
    ...
    auto add_scan_results_tlv_to_report =
        [this, &set_neighbor_in_scan_results_tlv](
            ...
            std::chrono::system_clock::time_point timestamp, uint32_t utilization, uint32_t noise,
        ...
        int total_noise       = noise;
        int total_utilization = utilization;
        ...
        results_tlv->noise() =
            neighbors_list_length == 0 ? total_noise : (total_noise / neighbors_list_length);
        results_tlv->utilization() = neighbors_list_length == 0
                                         ? total_utilization
                                         : (total_utilization / neighbors_list_length);
        ...
        [&final_results](bool fresh_scan_requested, const sMacAddr &ruid,
            ...
            if (!fresh_scan_requested ||
                ...
                auto it              = std::find_if(results.begin(), results.end(),
                                       [](const beerocks_message::sChannelScanResults &results) {
                                           return std::strlen(results.ssid) == 0;
                                       });
