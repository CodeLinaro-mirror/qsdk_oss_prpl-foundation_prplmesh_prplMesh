...
private:
    ...
    */
    ...
    bool ap_autoconfiguration_wsc_parse_encrypted_settings(
        ...
        uint8_t keywrapkey[16], WSC::EncryptedSettingsPayload::config &payload_config);
    ...
    /**
     * @brief Configure missing MLD SSIDs during auto-configuration phase.
     *
     * For SSIDs in bss_infos that don't have MLD configuration, this function
     * configures them with DISABLED_MLDUNIT.
     *
     * @param [in] radio_iface Radio interface name.
     * @param [in] bss_infos Vector of BSS configurations.
     */
    void configure_missing_mld_ssids(const std::string &radio_iface,
