...
private:
    ...
    */
    ...
    /**
     * @brief Select the preconfigured or an available MLD unit for an AP MLD.
     *
     * @param ssid AP MLD SSID used to match a preconfigured unit.
     * @param affiliated_ruids Radios belonging to the requested AP MLD.
     *
     * @return Selected MLD unit ID (0 to max_mlds-1), or DISABLED_MLDUNIT if none available.
     */
    int8_t select_ap_mld_unit(const std::string &ssid,
