...
bool is_afc_regulatory_preference_applicable(AgentDB::sRadio *radio)
{
    if (!radio) {
        return false;
    }
    ...
    return it != db->afc_radio_platform_data.end() && it->second.data_valid &&
           it->second.regulatory_applicable;
}

bool read_possible_channels_from_db(AgentDB::sRadio *radio, std::unordered_set<uint8_t> &channels)
{
    channels.clear();
    if (!radio) {
        return false;
    ...
    channels = it->second.possible_channels;
    ...
    std::unordered_map<uint8_t, bool> *beacon_operable_snapshot_out = nullptr)
{
    ...
    const bool use_dm_allowed_channels =
        read_possible_channels_from_db(radio, afc_allowed_channels);
...
bool ChannelSelectionTask::build_channel_preference_report(const sMacAddr &radio_mac)
{
    ...
    const bool apply_afc_regulatory_restrictions =
        ...
        is_afc_regulatory_preference_applicable(radio);
...
bool ChannelSelectionTask::build_afc_channel_preference_report(const sMacAddr &radio_mac)
{
    ...
    if (!is_afc_regulatory_preference_applicable(radio)) {
