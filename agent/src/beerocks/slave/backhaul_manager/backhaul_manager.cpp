...
bool BackhaulManager::backhaul_fsm_wireless(bool &skip_select)
{
    ...
    case EState::WPA_ATTACH: {
        ...
        if (provisioned) {
            const int num_connected = std::accumulate(
                m_radios_info.begin(),
                m_radios_info.end(),
                false,
                [](bool acc, const std::shared_ptr<sRadioInfo> &radio_info) {
                    return acc + ((radio_info->sta_wlan_hal && radio_info->sta_wlan_hal->is_connected()) ? 1 : 0);
                });
...
bool BackhaulManager::retry_steer_scan()
{
    ...
    m_backhaul_steering_scan_retry_timer = m_timer_manager->add_timer(
        "Backhaul Steering Scan Retry",
        backhaul_steering_scan_retry,
        std::chrono::milliseconds::zero(),
        [&](int fd, beerocks::EventLoop &loop) {
            auto iface_hal = get_wireless_hal(m_backhaul_steering_iface);
            if (!iface_hal) {
                LOG(ERROR) << "Failed to get iface_hal for iface "
                           << m_backhaul_steering_iface
                           << " to restart steering scan";
                return true;
            }

            const bool scan_started = iface_hal->scan_bss(
                    m_backhaul_steering_bssid,
                    m_backhaul_steering_channel.first,
                                        ...
                                        std::string iface)
{
    ...
    case Event::ScanResults: {
        ...
        if (FSM_IS_IN_STATE(OPERATIONAL) &&
            ...
            const int bsses_count = iface_hal->get_scan_results(db->device_conf.back_radio.ssid, bsses);
            const bool bssid_found = std::find_if(
                bsses.begin(), bsses.end(),
                [&](const bwl::sScanResult &bss) {
...
bool BackhaulManager::reconnect()
{
    ...
    auto db = AgentDB::get();
    const bool ok = active_hal->connect(
        db->device_conf.back_radio.ssid,
        db->device_conf.back_radio.pass,
        db->device_conf.back_radio.security_type,
        db->device_conf.back_radio.mem_only_psk,
