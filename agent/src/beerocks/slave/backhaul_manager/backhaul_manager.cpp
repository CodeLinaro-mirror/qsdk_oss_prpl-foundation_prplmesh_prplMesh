                                        ...
                                        std::string iface)
{
    ...
    case Event::ScanFailed: {
        ...
        if (FSM_IS_IN_STATE(OPERATIONAL) &&
            ...
            LOG(DEBUG)
                << "Received scan failed while a steering bssid is set. Scheduling another scan";
    ...
    case Event::ScanResults: {
        ...
        if (FSM_IS_IN_STATE(OPERATIONAL) &&
            ...
            if (iface != m_backhaul_steering_iface) {
                LOG(DEBUG) << "Scan results are for iface " << iface << ", but steering iface is "
                           << m_backhaul_steering_iface << ", ignoring event";
                return true;
            }
            ...
            if (different_radio_switch) {
                LOG(DEBUG) << "Steering requires a radio switch"
                           << " from iface " << active_hal->get_iface_name() << " to iface "
                           << iface_hal->get_iface_name();
                ...
                associate = iface_hal->connect(db->device_conf.back_radio.ssid,
                                                       ...
                                                       const sMacAddr &src_mac)
{
    ...
    const auto sta_iface_it = std::find_if(
        m_radios_info.begin(), m_radios_info.end(),
        [&sta_addr](const auto &radio_info) { return radio_info->radio_mac == sta_addr; });
    if (sta_iface_it == m_radios_info.end()) {
        LOG(WARNING) << "Unable to steer to BSSID " << bssid << ": No radio found for STA address "
                     << sta_addr;

        auto response = create_backhaul_steering_response(
            wfa_map::tlvErrorCode::eReasonCode::CLIENT_CAPABILITY_REPORT_UNSPECIFIED_FAILURE,
            bssid);
    ...
    LOG(DEBUG) << "Backhaul Steering Request parameters: "
               ...
               << " Operating Class=" << int(oper_class) << " Channel=" << int(channel)
