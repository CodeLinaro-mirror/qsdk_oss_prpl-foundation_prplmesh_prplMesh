...
class tlvTIDtoLinkMappingPolicy : public BaseClass
{
    public:
        ...
        std::tuple<bool, cNumTIDtoLink&> num_tid_to_link_mappings(size_t idx);
        std::shared_ptr<cNumTIDtoLink> create_num_tid_to_link_mappings();
        bool add_num_tid_to_link_mappings(std::shared_ptr<cNumTIDtoLink> ptr);
    ...
    private:
        ...
        cNumTIDtoLink* m_num_tid_to_link_mappings = nullptr;
        size_t m_num_tid_to_link_mappings_idx__ = 0;
        std::vector<std::shared_ptr<cNumTIDtoLink>> m_num_tid_to_link_mappings_vector;
        bool m_lock_allocation__ = false;
};

class cNumTIDtoLink : public BaseClass
{
    public:
        cNumTIDtoLink(uint8_t* buff, size_t buff_len, bool parse = false);
        explicit cNumTIDtoLink(std::shared_ptr<BaseClass> base, bool parse = false);
        ~cNumTIDtoLink();
