...
size_t tlvProfile2ChannelScanResult::scan_result_length()
{
    size_t len = 0;
    for (size_t i = 0; i < m_scan_result_idx__; i++) {
        len += m_scan_result_vector[i]->getLen();
    }
    return len;
}

std::tuple<bool, cRadioScanResult&> tlvProfile2ChannelScanResult::scan_result(size_t idx) {
    bool ret_success = ( (m_scan_result_idx__ > 0) && (m_scan_result_idx__ > idx) );
    size_t ret_idx = ret_success ? idx : 0;
    if (!ret_success) {
        TLVF_LOG(ERROR) << "Requested index is greater than the number of available entries";
    }
    return std::forward_as_tuple(ret_success, *(m_scan_result_vector[ret_idx]));
}

std::shared_ptr<cRadioScanResult> tlvProfile2ChannelScanResult::create_scan_result() {
    if (m_lock_order_counter__ > 0) {
        TLVF_LOG(ERROR) << "Out of order allocation for variable length list scan_result, abort!";
        return nullptr;
    }
    size_t len = cRadioScanResult::get_initial_size();
    ...
    uint8_t *src = (uint8_t *)m_scan_result;
    if (m_scan_result_idx__ > 0) {
        src = (uint8_t *)m_scan_result_vector[m_scan_result_idx__ - 1]->getBuffPtr();
    }
    if (!m_parse__) {
        uint8_t *dst = src + len;
    ...
    return std::make_shared<cRadioScanResult>(getBuffPtr(), getBuffRemainingBytes(), m_parse__);
}

bool tlvProfile2ChannelScanResult::add_scan_result(std::shared_ptr<cRadioScanResult> ptr) {
    if (ptr == nullptr) {
        TLVF_LOG(ERROR) << "Received entry is nullptr";
        return false;
    }
    if (m_lock_allocation__ == false) {
        TLVF_LOG(ERROR) << "No call to create_scan_result was called before add_scan_result";
        return false;
    }
    uint8_t *src = (uint8_t *)m_scan_result;
    if (m_scan_result_idx__ > 0) {
    ...
    m_scan_result_vector.push_back(ptr);
...
void tlvProfile2ChannelScanResult::class_swap()
{
    ...
    for (size_t i = 0; i < m_scan_result_idx__; i++){
        std::get<1>(scan_result(i)).class_swap();
    }
}
...
bool tlvProfile2ChannelScanResult::init()
{
    ...
    m_scan_result = reinterpret_cast<cRadioScanResult*>(m_buff_ptr__);
    if (m_length && m_parse__) {
        ...
        while (len > 0) {
            if (len < cRadioScanResult::get_initial_size()) {
                TLVF_LOG(ERROR) << "Invalid length (scan_result)";
                return false;
            }
            auto scan_result = create_scan_result();
            if (!scan_result || !scan_result->isInitialized()) {
                TLVF_LOG(ERROR) << "create_scan_result() failed";
                return false;
            }
            if (!add_scan_result(scan_result)) {
                TLVF_LOG(ERROR) << "add_scan_result() failed";
                return false;
            }
            // swap back since scan_result will be swapped as part of the whole class swap
            scan_result->class_swap();
            len -= scan_result->getLen();
        }
    }
...
cRadioScanResult::cRadioScanResult(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cRadioScanResult::cRadioScanResult(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cRadioScanResult::~cRadioScanResult() {
}
uint8_t& cRadioScanResult::timestamp_length() {
    return (uint8_t&)(*m_timestamp_length);
}

std::string cRadioScanResult::timestamp_str() {
...
char* cRadioScanResult::timestamp(size_t length) {
...
bool cRadioScanResult::set_timestamp(const std::string& str) { return set_timestamp(str.c_str(), str.size()); }
bool cRadioScanResult::set_timestamp(const char str[], size_t size) {
    if (str == nullptr) {
...
bool cRadioScanResult::alloc_timestamp(size_t count) {
    if (m_lock_order_counter__ > 0) {;
...
uint8_t& cRadioScanResult::utilization() {
    return (uint8_t&)(*m_utilization);
}

uint8_t& cRadioScanResult::noise() {
    return (uint8_t&)(*m_noise);
}

uint16_t& cRadioScanResult::neighbors_list_length() {
    return (uint16_t&)(*m_neighbors_list_length);
}

std::tuple<bool, cNeighbors&> cRadioScanResult::neighbors_list(size_t idx) {
...
std::shared_ptr<cNeighbors> cRadioScanResult::create_neighbors_list() {
...
bool cRadioScanResult::add_neighbors_list(std::shared_ptr<cNeighbors> ptr) {
    if (ptr == nullptr) {
        TLVF_LOG(ERROR) << "Received entry is nullptr";
        return false;
    }
...
uint32_t& cRadioScanResult::aggregate_scan_duration() {
    return (uint32_t&)(*m_aggregate_scan_duration);
}

cRadioScanResult::eScanType& cRadioScanResult::scan_type() {
    return (eScanType&)(*m_scan_type);
}

void cRadioScanResult::class_swap()
{
...
bool cRadioScanResult::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cRadioScanResult::get_initial_size()
{
    size_t class_size = 0;
