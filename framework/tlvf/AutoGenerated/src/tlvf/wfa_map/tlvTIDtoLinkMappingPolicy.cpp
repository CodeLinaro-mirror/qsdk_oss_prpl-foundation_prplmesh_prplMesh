...
std::tuple<bool, cNumTIDtoLink&> tlvTIDtoLinkMappingPolicy::num_tid_to_link_mappings(size_t idx) {
...
std::shared_ptr<cNumTIDtoLink> tlvTIDtoLinkMappingPolicy::create_num_tid_to_link_mappings() {
    if (m_lock_order_counter__ > 0) {
        TLVF_LOG(ERROR) << "Out of order allocation for variable length list num_tid_to_link_mappings, abort!";
        return nullptr;
    }
    size_t len = cNumTIDtoLink::get_initial_size();
    ...
    return std::make_shared<cNumTIDtoLink>(src, getBuffRemainingBytes(src), m_parse__);
}

bool tlvTIDtoLinkMappingPolicy::add_num_tid_to_link_mappings(std::shared_ptr<cNumTIDtoLink> ptr) {
    if (ptr == nullptr) {
...
bool tlvTIDtoLinkMappingPolicy::init()
{
    ...
    m_num_tid_to_link_mappings = reinterpret_cast<cNumTIDtoLink*>(m_buff_ptr__);
...
cNumTIDtoLink::cNumTIDtoLink(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cNumTIDtoLink::cNumTIDtoLink(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cNumTIDtoLink::~cNumTIDtoLink() {
}
cNumTIDtoLink::sFlags4& cNumTIDtoLink::flags() {
    return (sFlags4&)(*m_flags);
}

sMacAddr& cNumTIDtoLink::sta_mld_mac_addr() {
    return (sMacAddr&)(*m_sta_mld_mac_addr);
}

uint8_t& cNumTIDtoLink::link_mapping_presence_indicator() {
    return (uint8_t&)(*m_link_mapping_presence_indicator);
}

uint8_t* cNumTIDtoLink::expected_duration(size_t idx) {
...
bool cNumTIDtoLink::set_expected_duration(const void* buffer, size_t size) {
    if (buffer == nullptr) {
...
uint16_t* cNumTIDtoLink::tid_to_link_mappings(size_t idx) {
...
bool cNumTIDtoLink::alloc_tid_to_link_mappings(size_t count) {
    if (m_lock_order_counter__ > 0) {;
...
uint8_t* cNumTIDtoLink::reserved(size_t idx) {
    if ( (m_reserved_idx__ == 0) || (m_reserved_idx__ <= idx) ) {
        TLVF_LOG(ERROR) << "Requested index is greater than the number of available entries";
        return nullptr;
    }
...
bool cNumTIDtoLink::set_reserved(const void* buffer, size_t size) {
    if (buffer == nullptr) {
        TLVF_LOG(WARNING) << "set_reserved received a null pointer.";
        return false;
    }
...
size_t cNumTIDtoLink::get_initial_size()
{
    size_t class_size = 0;
