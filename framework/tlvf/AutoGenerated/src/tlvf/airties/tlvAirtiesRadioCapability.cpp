...
tlvAirtiesRadioCapability::tlvAirtiesRadioCapability(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
tlvAirtiesRadioCapability::tlvAirtiesRadioCapability(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
tlvAirtiesRadioCapability::~tlvAirtiesRadioCapability() {
}
const eAirtiesTlvTypeMap& tlvAirtiesRadioCapability::type() {
    return (const eAirtiesTlvTypeMap&)(*m_type);
}

const uint16_t& tlvAirtiesRadioCapability::length() {
    return (const uint16_t&)(*m_length);
}

sVendorOUI& tlvAirtiesRadioCapability::vendor_oui() {
    return (sVendorOUI&)(*m_vendor_oui);
}

uint16_t& tlvAirtiesRadioCapability::tlv_id() {
    return (uint16_t&)(*m_tlv_id);
}

sMacAddr& tlvAirtiesRadioCapability::radio_id() {
    return (sMacAddr&)(*m_radio_id);
}

tlvAirtiesRadioCapability::sStandards& tlvAirtiesRadioCapability::standards() {
    return (sStandards&)(*m_standards);
}

uint8_t& tlvAirtiesRadioCapability::standards_reserved() {
    return (uint8_t&)(*m_standards_reserved);
}

uint16_t& tlvAirtiesRadioCapability::reserved() {
    return (uint16_t&)(*m_reserved);
}

void tlvAirtiesRadioCapability::class_swap()
{
    tlvf_swap(16, reinterpret_cast<uint8_t*>(m_length));
    m_vendor_oui->struct_swap();
    tlvf_swap(16, reinterpret_cast<uint8_t*>(m_tlv_id));
    m_radio_id->struct_swap();
    m_standards->struct_swap();
    tlvf_swap(16, reinterpret_cast<uint8_t*>(m_reserved));
}

bool tlvAirtiesRadioCapability::finalize()
{
    if (m_parse__) {
...
size_t tlvAirtiesRadioCapability::get_initial_size()
{
    size_t class_size = 0;
    ...
    class_size += sizeof(sStandards); // standards
    class_size += sizeof(uint8_t); // standards_reserved
    class_size += sizeof(uint16_t); // reserved
    return class_size;
}

bool tlvAirtiesRadioCapability::init()
{
    ...
    m_standards = reinterpret_cast<sStandards*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(sStandards))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(sStandards) << ") Failed!";
        return false;
    }
    if(m_length && !m_parse__){ (*m_length) += sizeof(sStandards); }
    if (!m_parse__) { m_standards->struct_init(); }
    m_standards_reserved = reinterpret_cast<uint8_t*>(m_buff_ptr__);
