...
eVapType& cEncryptedSettingsPayload::vap_type() {
...
bool cEncryptedSettingsPayload::init()
{
    ...
    if (!m_parse__) *m_bss_index = 0x0;
    if (!buffPtrIncrementSafe(sizeof(int8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(int8_t) << ") Failed!";
        return false;
    }
    m_additional_auth = reinterpret_cast<uint8_t*>(m_buff_ptr__);
    if (!m_parse__) *m_additional_auth = 0x0;
    if (!buffPtrIncrementSafe(sizeof(uint8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint8_t) << ") Failed!";
        return false;
    }
    m_vap_type = reinterpret_cast<eVapType*>(m_buff_ptr__);
...
cWscAttrBssIndex::cWscAttrBssIndex(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cWscAttrBssIndex::cWscAttrBssIndex(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cWscAttrBssIndex::~cWscAttrBssIndex() {
}
eWscAttributes& cWscAttrBssIndex::type() {
    return (eWscAttributes&)(*m_type);
}

const uint16_t& cWscAttrBssIndex::length() {
    return (const uint16_t&)(*m_length);
}

uint8_t& cWscAttrBssIndex::index() {
    return (uint8_t&)(*m_index);
}

void cWscAttrBssIndex::class_swap()
{
    tlvf_swap(16, reinterpret_cast<uint8_t*>(m_type));
    tlvf_swap(16, reinterpret_cast<uint8_t*>(m_length));
}

bool cWscAttrBssIndex::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cWscAttrBssIndex::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(eWscAttributes); // type
    class_size += sizeof(uint16_t); // length
    class_size += sizeof(uint8_t); // index
    return class_size;
}

bool cWscAttrBssIndex::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
    ...
    m_type = reinterpret_cast<eWscAttributes*>(m_buff_ptr__);
    if (!m_parse__) *m_type = ATTR_BSS_INDEX;
    if (!buffPtrIncrementSafe(sizeof(eWscAttributes))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(eWscAttributes) << ") Failed!";
        return false;
    }
    ...
    m_index = reinterpret_cast<uint8_t*>(m_buff_ptr__);
    if (!m_parse__) *m_index = 0x0;
    if (!buffPtrIncrementSafe(sizeof(uint8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint8_t) << ") Failed!";
        return false;
    }
