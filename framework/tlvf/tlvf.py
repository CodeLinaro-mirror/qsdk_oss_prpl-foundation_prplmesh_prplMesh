...
class TlvF:
    def __init__(self, src_path, yaml_path, out_path, conf_path, print_dependencies, print_outputs,
                 flash_optimization=False):
        # Off by default: the committed AutoGenerated is the unoptimized shape.
        self.flash_optimization = flash_optimization
        self.log_macro = "TLVF_LOG" if flash_optimization else "LOG"
        self.init_decl_prefix = "TLVF_LOCAL " if flash_optimization else ""
    ...
    def addClassParam(self, obj_meta, param_name, param_type, param_type_info, param_meta=None):
        ...
        if param_length_type == None:
            if TypeInfo(param_type).type == TypeInfo.CLASS:
                ...
                lines_cpp.append("m_%s = reinterpret_cast<%s*>(m_%s__);" %
                                 (param_name, param_type, self.MEMBER_BUFF_PTR))
                lines_cpp.extend(self.guardBuffPtrIncrement(
                    0, "sizeof(%s)" % param_type,
                    condition=param_meta.condition[MetaData.CONDITION_PHRASE]))
                ...
                lines_cpp.append("%sif (!m_%s_allocated && !alloc_%s()) {" % (
                    self.getIndentation(1), param_name, param_name))
                lines_cpp.append('%s%s(ERROR) << "Could not allocate %s!";' % (
                    self.getIndentation(2), self.log_macro, param_name))
                lines_cpp.append('%sreturn false;' % self.getIndentation(2))
                lines_cpp.append("%s}" % self.getIndentation(1))
                ...
                if param_class_const:
                    ...
                    if obj_meta.is_tlv_class and param_name == MetaData.TLV_TYPE_LENGTH:
                        ...
                        lines_cpp.append("if (!m_%s__) *m_%s = 0;" %
                                         (self.MEMBER_PARSE, param_name))
                    lines_cpp.extend(self.guardBuffPtrIncrement(0, "sizeof(%s)" % param_type))
                    lines_cpp.append("%sreturn false;" % (self.getIndentation(1)))
                    lines_cpp.append("%s}" % (self.getIndentation(0)))
            ...
            if is_dynamic_len:
                ...
                if TypeInfo(param_type).type == TypeInfo.CLASS:
                    ...
                    lines_cpp.extend(self.guardBuffPtrIncrement(1, "len"))
                    lines_cpp.append("%sreturn false;" % (self.getIndentation(2)))
                    lines_cpp.append("%s}" % (self.getIndentation(1)))
                lines_cpp.append("}")
            ...
            if is_var_len:
                ...
                if TypeInfo(param_type).type == TypeInfo.CLASS:
                    ...
                    lines_cpp.extend(self.guardBuffPtrIncrement(
                        0, "sizeof(%s) * (%s)" % (param_type, param_length)))
                    lines_cpp.append("%sreturn false;" % (self.getIndentation(1)))
                    lines_cpp.append("%s}" % (self.getIndentation(0)))
    ...
    def addClassVarLenMethods(self, obj_meta, param_type, param_name, param_meta, param_length, is_var_len, is_dynamic_len):
        ...
        if TypeInfo(param_type).type == TypeInfo.CLASS:
            ...
            lines_cpp.append("%sreturn nullptr;" % self.getIndentation(2))
            lines_cpp.append("%s}" % self.getIndentation(1))
            lines_cpp.extend(self.guardHasSpace(
                1, "hasSpaceFor(len)",
                "%sif (getBuffRemainingBytes() < len) {" % self.getIndentation(1),
                "Not enough available space on buffer"))
            lines_cpp.append("%sreturn nullptr;" % self.getIndentation(2))
            lines_cpp.append("%s}" % self.getIndentation(1))
            ...
            lines_cpp.extend(self.guardHasSpace(
                1, "hasSpaceFor(ptr->getLen(), ptr->getStartBuffPtr())",
                "%sif (ptr->getLen() > getBuffRemainingBytes(ptr->getStartBuffPtr())) {;"
                % self.getIndentation(1),
                "Not enough available space on buffer"))
            lines_cpp.append("%sreturn false;" % self.getIndentation(2))
            lines_cpp.append("%s}" % self.getIndentation(1))
            ...
            lines_cpp.append('%s%s(ERROR) << "%s already allocated!";' % (
                self.getIndentation(2), self.log_macro, param_name))
            lines_cpp.append('%sreturn false;' % self.getIndentation(2))
            lines_cpp.append('%s}' % self.getIndentation(1))
            ...
            lines_cpp.append("%ssize_t len = sizeof(%s);" %
                             (self.getIndentation(1), param_type))
            lines_cpp.extend(self.guardHasSpace(
                1, "hasSpaceForAlloc(len)",
                "%sif(getBuffRemainingBytes() < len )  {" % self.getIndentation(1),
                "Not enough available space on buffer - can't allocate"))
            lines_cpp.append("%sreturn false;" % self.getIndentation(2))
            lines_cpp.append("%s}" % self.getIndentation(1))
    ...
    def addClassInitMethod(self, insert_name, insert_marker, name):
        self.insertLineH(insert_name, insert_marker,
                         "%s%sbool init();" % (self.getIndentation(2), self.init_decl_prefix))

        self.insertLineCpp(insert_name, insert_marker, "bool %s::init()" % (name))
        self.insertLineCpp(insert_name, insert_marker, "{")
        for guard_line in self.guardHasSpace(
                1, "hasInitialSpace(get_initial_size())",
                "%sif (getBuffRemainingBytes() < get_initial_size()) {" % self.getIndentation(1),
                "Not enough available space on buffer. Class init failed"):
            self.insertLineCpp(insert_name, insert_marker, guard_line)
    ...
    def guardBuffPtrIncrement(self, indent, length_expr, condition=None):
        # Opening of a buffPtrIncrementSafe() guard; the caller adds the body.
        cond = "(%s) && " % condition if condition else ""
        if self.flash_optimization:
            return ["%sif (%s!buffPtrIncrementSafeLogged(%s)) {" % (
                self.getIndentation(indent), cond, length_expr)]
        return [
            "%sif (%s!buffPtrIncrementSafe(%s)) {" % (
                self.getIndentation(indent), cond, length_expr),
            "%s%s(ERROR) << \"buffPtrIncrementSafe(\" << std::dec << %s << \") Failed!\";" % (
                self.getIndentation(indent + 1), self.log_macro, length_expr)]

    def guardHasSpace(self, indent, optimized_call, stock_line, message):
        # stock_line is passed verbatim, odd spacing and all, so that the
        # unoptimized output stays byte-identical to the previous generator.
        if self.flash_optimization:
            return ["%sif (!%s) {" % (self.getIndentation(indent), optimized_call)]
        return [stock_line,
                '%sTLVF_LOG(ERROR) << "%s";' % (self.getIndentation(indent + 1), message)]
...
def main():
    ...
    parser.add_argument('--flash-optimization', action='store_true',
                        help='emit codec guards as calls to the out-of-line BaseClass helpers '
                             'instead of inline check-and-log pairs')
