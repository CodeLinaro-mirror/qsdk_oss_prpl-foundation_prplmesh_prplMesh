...
{
    if (!map) {
        LOG(ERROR) << "DSCP map is null";
        return false;
    }

    const auto pcp = map->dscp[map->precendence]; // pcp should be [0;7]
    if (pcp > 7) {
        LOG(WARNING) << "The mapped pcp value is out of range. Default pcp is used - "
                     << static_cast<int>(default_pcp);
        apply_rules(*iface_list, default_pcp);
    } else {
        LOG(DEBUG) << "Applying dscp map | PCP = " << pcp;
        apply_rules(*iface_list, pcp);
    }
