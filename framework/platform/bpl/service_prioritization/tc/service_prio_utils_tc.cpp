...
constexpr const char *kClsactQdisc          = "clsact";
...
bool run_tc_command(const std::string &cmd)
{
    if (cmd.empty()) {
...
bool ServicePrioritizationUtils_tc::flush_rules()
{
    ...
    for (const auto &iface_name : interfaces)
        success = remove_filters(iface_name) && success;
                                                           ...
                                                           uint8_t pcp)
{
    ...
    for (const auto &iface : *iface_list) {
        ...
        // Install VLAN-specific rules first so tagged traffic does not fall through to the
        // generic matchall skb priority rule.
...
{
    std::string cmd =
        std::string(kTcBinary) + " qdisc replace dev " + iface_name + " " + kClsactQdisc;
    if (!run_tc_command(cmd)) {
        return false;
    }
...
bool ServicePrioritizationUtils_tc::remove_filters(const std::string &iface_name)
{
    ...
    for (auto pref : iface_it->second) {
        ...
        cmd << kTcBinary << " filter del dev " << iface_name << " " << kEgressSelector << " pref "
            << pref;
        success = run_tc_command(cmd.str()) && success;
    }
