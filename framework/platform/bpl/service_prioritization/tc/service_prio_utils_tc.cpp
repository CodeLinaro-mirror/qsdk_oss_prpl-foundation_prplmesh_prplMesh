                                                           ...
                                                           uint8_t pcp)
{
    ...
    for (const auto &iface : *iface_list) {
        ...
        // Apply the filter for traffic with VLAN ID 10
        cmd = "tc filter add dev " + iface.iface_name +
              " parent ffff: protocol 802.1Q flower vlan_id 10 action goto chain 1";
        beerocks::os_utils::system_call(cmd);

        cmd = "tc filter add dev " + iface.iface_name +
              " chain 1 parent ffff: protocol 802.1Q matchall action vlan "
              "modify priority " +
              std::to_string(pcp) + " id 10";
        beerocks::os_utils::system_call(cmd);
    }
