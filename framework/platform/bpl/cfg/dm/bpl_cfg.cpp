...
{
    sta_reporting_rcpi_threshold = DEFAULT_STA_REPORTING_RCPI_THRESHOLD;
    return true;
}
    ...
    unsigned int &sta_reporting_rcpi_hyst_margin_override_threshold)
{
    sta_reporting_rcpi_hyst_margin_override_threshold =
        DEFAULT_STA_REPORTING_RCPI_HYSTERESIS_MARGIN_OVERRIDE_THRESHOLD;
    return true;
}
...
{
    ap_reporting_channel_utilization_threshold = DEFAULT_AP_REPORTING_CHANNEL_UTILIZATION_THRESHOLD;
    return true;
}
...
{
    steering_policy = DEFAULT_STEERING_POLICY;
    return true;
}
