...
std::unordered_map<sMacAddr /* al_mac */, std::shared_ptr<SteerWiFiBackhaulRequest>>
    g_steer_wifi_backhaul_requests;
...
static const std::shared_ptr<db::Agent::sRadio>
find_agent_radio_with_channel_capability(const eFreqType freq_type, const uint8_t primary_channel)
{
    const auto agents = g_database->get_all_connected_agents();
    for (const auto &agent : agents) {
        for (const auto &radio_it : agent->radios) {
            const auto radio = radio_it.second;
            if (std::find_if(radio->supported_channels.begin(), radio->supported_channels.end(),
...
struct SteerWiFiBackhaulRequest {
    ...
    SteerWiFiBackhaulRequest(const sMacAddr &al_mac, uint64_t call_id, int32_t timeout_msec)
        : m_al_mac(al_mac), m_call_id(call_id), m_timeout_fd(-1)
    {
        ...
        m_timeout_fd =
            g_timer_manager->add_timer("Steering Backhaul Response timeout", delay, repeat, cb);
                                     ...
                                     const sMacAddr &target_bssid, const bool success)
{
    ...
    const char *status_str =
        success ? STEER_WIFI_BACKHAUL_STATUS_SUCCESS : STEER_WIFI_BACKHAUL_STATUS_ERROR_OTHER;
 ...
 *
 */
amxd_status_t steer_wifi_backhaul(amxd_object_t *object, amxd_function_t *func, amxc_var_t *args,
                                  amxc_var_t *ret)
{
    auto controller_ctx = g_database->get_controller_ctx();
    ...
    if (!controller_ctx) {
        ...
        LOG(ERROR) << reason;
        amxc_var_add_key(cstring_t, ret, STEER_WIFI_BACKHAUL_STATUS_KEY,
                         STEER_WIFI_BACKHAUL_STATUS_ERROR_NOT_READY);
    ...
    if (!target_bssid_arg) {
        ...
        LOG(ERROR) << reason;
        amxc_var_add_key(cstring_t, ret, STEER_WIFI_BACKHAUL_STATUS_KEY,
                         STEER_WIFI_BACKHAUL_STATUS_ERROR_INVALID_INPUT);
        amxc_var_add_key(cstring_t, ret, X_PRPLWARE_REASON, reason.c_str());
        return amxd_status_parameter_not_found;
    }

    if (!network_utils::is_valid_mac(target_bssid_arg)) {
        const std::string reason("TargetBSS: " + std::string(target_bssid_arg) +
                                 " is not a valid MAC address");
        LOG(ERROR) << reason;
        amxc_var_add_key(cstring_t, ret, STEER_WIFI_BACKHAUL_STATUS_KEY,
                         STEER_WIFI_BACKHAUL_STATUS_ERROR_INVALID_INPUT);
    ...
    if (already_requesting) {
        const std::string reason("Steer WiFi Backhaul request is already in progress for AL: " +
                                 al_mac_str);
        amxc_var_add_key(cstring_t, ret, STEER_WIFI_BACKHAUL_STATUS_KEY,
                         STEER_WIFI_BACKHAUL_STATUS_ERROR_NOT_READY);
        amxc_var_add_key(cstring_t, ret, X_PRPLWARE_REASON, reason.c_str());
    ...
    if (!controller_ctx->send_backhaul_steering_request(al_mac, backhaul_sta_mac, target_bssid,
                                                        target_op_class, target_channel)) {
    ...
    if (deferred != amxd_status_ok) {
        const std::string reason(
            "amxd_function_defer failed for Steer WiFi Backhaul request for AL: " + al_mac_str);
        LOG(ERROR) << reason;
        amxc_var_add_key(cstring_t, ret, STEER_WIFI_BACKHAUL_STATUS_KEY,
                         STEER_WIFI_BACKHAUL_STATUS_ERROR_OTHER);
        amxc_var_add_key(cstring_t, ret, X_PRPLWARE_REASON, reason.c_str());
...
std::vector<beerocks::nbapi::sFunctions> get_func_list(void)
{
    const std::vector<beerocks::nbapi::sFunctions> functions_list = {
        ...
        {"remove_unassociated_station",
         ...
         remove_unassociated_station},
        {"steer_wifi_backhaul",
         DATAELEMENTS_ROOT_DM ".Network.Device.MultiAPDevice.Backhaul.SteerWiFiBackhaul",
         steer_wifi_backhaul},
