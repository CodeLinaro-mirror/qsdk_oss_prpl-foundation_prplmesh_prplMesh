                                        ...
                                        amxc_var_t *args, amxc_var_t *ret)
{
    ...
    if (!controller_ctx) {
        ...
        chan_sel_req_unknown_error(ret, reason.str().c_str());
        return amxd_status_ok;
    }
    ...
    const amxc_var_t *classes = GET_ARG(ret, "Class");
    if (!chan_sel_req_parse_classes(classes, &radio_preferences, &reason)) {
        chan_sel_req_invalid_param(ret, reason.str().c_str());
        return amxd_status_ok;
    }
    ...
    if (!controller_ctx->trigger_channel_selection_request(al_mac, preferences)) {
        chan_sel_req_unknown_error(ret, "Controller failed to trigger channel selection request");
        return amxd_status_ok;
    }

    chan_sel_req_success(ret);
    return amxd_status_ok;
}
