...
TEST(afc_easymesh_tlv, round_trip_beerocks_afc_update_notification_payload)
{
    const size_t buf_size = beerocks::message::MESSAGE_BUFFER_LENGTH;
    uint8_t tx_buffer[beerocks::message::MESSAGE_BUFFER_LENGTH];
    std::memset(tx_buffer, 0, buf_size);

    beerocks_message::cACTION_APMANAGER_AFC_UPDATE_NOTIFICATION tx_msg(tx_buffer, buf_size, false);
    ASSERT_TRUE(tx_msg.isPostInitSucceeded());

    const std::string inquiry_request = R"({"availableSpectrumInquiryRequests":[{"requestId":1}]})";
    const std::string inquiry_response =
        R"({"availableChannelInfo":[{"globalOperatingClass":131}]})";
    const std::string possible_channels = "5,21,37";

    tx_msg.grant_successful()      = 1;
    tx_msg.regulatory_applicable() = 1;
    ASSERT_TRUE(tx_msg.set_inquiry_request(inquiry_request));
    ASSERT_TRUE(tx_msg.set_inquiry_response(inquiry_response));
    ASSERT_TRUE(tx_msg.set_possible_channels(possible_channels));
    ASSERT_TRUE(tx_msg.finalize());

    uint8_t rx_buffer[beerocks::message::MESSAGE_BUFFER_LENGTH];
    std::memcpy(rx_buffer, tx_buffer, tx_msg.getLen());

    beerocks_message::cACTION_APMANAGER_AFC_UPDATE_NOTIFICATION rx_msg(rx_buffer, buf_size, true);
    ASSERT_TRUE(rx_msg.isPostInitSucceeded());
    EXPECT_EQ(rx_msg.grant_successful(), 1U);
    EXPECT_EQ(rx_msg.regulatory_applicable(), 1U);
    EXPECT_EQ(rx_msg.inquiry_request_str(), inquiry_request);
    EXPECT_EQ(rx_msg.inquiry_response_str(), inquiry_response);
    EXPECT_EQ(rx_msg.possible_channels_str(), possible_channels);

    uint8_t bhm_buffer[beerocks::message::MESSAGE_BUFFER_LENGTH];
    std::memset(bhm_buffer, 0, buf_size);
    beerocks_message::cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION bhm_tx(bhm_buffer, buf_size, false);
    ASSERT_TRUE(bhm_tx.isPostInitSucceeded());
    bhm_tx.grant_successful()      = rx_msg.grant_successful();
    bhm_tx.regulatory_applicable() = rx_msg.regulatory_applicable();
    ASSERT_TRUE(bhm_tx.set_inquiry_request(rx_msg.inquiry_request_str()));
    ASSERT_TRUE(bhm_tx.set_inquiry_response(rx_msg.inquiry_response_str()));
    ASSERT_TRUE(bhm_tx.set_possible_channels(rx_msg.possible_channels_str()));
    ASSERT_TRUE(bhm_tx.finalize());

    uint8_t bhm_rx_buffer[beerocks::message::MESSAGE_BUFFER_LENGTH];
    std::memcpy(bhm_rx_buffer, bhm_buffer, bhm_tx.getLen());
    beerocks_message::cACTION_BACKHAUL_AFC_UPDATE_NOTIFICATION bhm_rx(bhm_rx_buffer, buf_size,
                                                                      true);
    ASSERT_TRUE(bhm_rx.isPostInitSucceeded());
    EXPECT_EQ(bhm_rx.inquiry_request_str(), inquiry_request);
    EXPECT_EQ(bhm_rx.inquiry_response_str(), inquiry_response);
    EXPECT_EQ(bhm_rx.possible_channels_str(), possible_channels);
}
