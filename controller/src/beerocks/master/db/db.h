 ...
 */
class single_shot_counter {
    unsigned counter = 0;
    std::function<void()> callback;

public:
    single_shot_counter() = default;
    single_shot_counter(unsigned initial, std::function<void()> f)
        : counter(initial), callback(std::move(f))
    {
    }

    ~single_shot_counter()                               = default;
    single_shot_counter(single_shot_counter &&) noexcept = default;
    single_shot_counter &operator=(single_shot_counter &&) noexcept = default;

    single_shot_counter(const single_shot_counter &) = delete;
    single_shot_counter &operator=(const single_shot_counter &) = delete;
...
public:
    ...
    struct ieee1905_network_db {
        ...
        struct sDmPathView {
            ...
            auto set(const std::string &parameter, const Value &value) const
             ...
             * @return result of Ambiorix::set(), or false if Ambiorix is gone
             */
            auto set(const std::string &parameter, const char *value) const
            {
             ...
             */
            sDmPath add_instance(const std::string &subpath) const;
        ...
        struct sAL {
            ...
            struct sNeighbor {
                sDmPath dm_path; ///< Owning data model path for this neighbor instance.
    ...
    */
     ...
     *        Data model path examples:
     * 
"Device.WiFi.DataElements.Network.Device.1.Radio.1.CurrentOperatingClasses"
     * 
"Device.WiFi.DataElements.Network.Device.1.MultiAPDevice.Backhaul.CurrentOperatingClassProfile".
    ...
    /**
     * @brief Create instances for each bandwidth possible for Radio.{i}.CurrentOperatingClasses and 
    ...
    /**
     * @brief Add optional sub-object of WIFI7 Agent Capabilities data element,
     * set values for its parameters.
     *
     * Example of full path to object:
     * "Device.WiFi.DataElements.Network.Device.1.Radio.1.Capabilities.WiFi7AgentCapabilities"
     *
     * @param wifi7_agt_caps_tlv TLV with WIFI7 Agent Capabilities included in
     * 'AP Capability Report' message
     * @return True if sub-object was successfully added
     * and values for its parameters set, false otherwise.
     */
...
private:
    ...
    int bml_task_id                          = -1;
    int ieee1905_task_id = -1; ///< Task ID of the running ieee1905_task, or -1 if not started.
