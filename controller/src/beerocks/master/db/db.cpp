...
bool db::dm_add_ap_mld(const sMacAddr &al_mac, Agent::sAPMLD &ap_mld)
{
    ...
    m_ambiorix_datamodel->set(ap_mld_config_dm_path, "STREnabled",
                              (ap_mld.mld_info.mld_mode & beerocks::MLO_MODE_STR) != 0);
    m_ambiorix_datamodel->set(ap_mld_config_dm_path, "NSTREnabled",
                              (ap_mld.mld_info.mld_mode & beerocks::MLO_MODE_NSTR) != 0);
    m_ambiorix_datamodel->set(ap_mld_config_dm_path, "EMLSREnabled",
                              (ap_mld.mld_info.mld_mode & beerocks::MLO_MODE_EMLSR) != 0);
    m_ambiorix_datamodel->set(ap_mld_config_dm_path, "EMLMREnabled",
                              (ap_mld.mld_info.mld_mode & beerocks::MLO_MODE_EMLMR) != 0);
...
bool db::dm_update_bsta_mld(const Agent &agent, const sMacAddr &bsta_mld_mac,
                            ...
                            const beerocks::eMLOModes &mld_mode)
{
    if (agent.dm_path.empty()) {
        return true;
    }
    ...
    if (!m_ambiorix_datamodel->set(bsta_mld_config, "STREnabled",
        ...
        LOG(ERROR) << "Failed to set STR " << beerocks::MLO_MODE_STR;
        ret_val &= false;
    }

    if (!m_ambiorix_datamodel->set(bsta_mld_config, "NSTREnabled",
                                   (mld_mode & beerocks::MLO_MODE_NSTR) != 0)) {
        LOG(ERROR) << "Failed to set NSTR " << beerocks::MLO_MODE_NSTR;
        ret_val &= false;
    }

    if (!m_ambiorix_datamodel->set(bsta_mld_config, "EMLSREnabled",
                                   (mld_mode & beerocks::MLO_MODE_EMLSR) != 0)) {
        LOG(ERROR) << "Failed to set EMLSR " << beerocks::MLO_MODE_EMLSR;
        ret_val &= false;
    }

    if (!m_ambiorix_datamodel->set(bsta_mld_config, "EMLMREnabled",
                                   (mld_mode & beerocks::MLO_MODE_EMLMR) != 0)) {
        LOG(ERROR) << "Failed to set EMLMR " << beerocks::MLO_MODE_EMLMR;
        ret_val &= false;
    }
    ...
    const beerocks::eMLOModes &mld_mode)
{
    ...
    if (station) {
        ...
        station->sta_mld_configuration.mld_config.mld_mode    = beerocks::eMLOModes(mld_mode);
                                  ...
                                  sMacAddr const &radio_mac_addr)
{
    ...
    auto add_un_station_dm = [&](const std::string &agent_mac, const std::string &radio_mac,
                                 const std::string &station_mac) -> bool {
        //example Device.WiFi.DataElements.Network.Device.1.Radio.2.UnassociatedSTA.
        std::string device_path = DATAELEMENTS_ROOT_DM ".Network.Device";
    ...
    using UnassociatedStationPtr = std::shared_ptr<UnassociatedStation>;
    ...
    auto add_update_unassociated_station_within_agent =
        [&](const std::shared_ptr<Agent> &agent) -> UnassociatedStationPtr {
        UnassociatedStationPtr new_updated_station(nullptr);
        std::shared_ptr<Agent::sRadio> agent_radio(nullptr);
        ...
        if (!agent_radio) {
            ...
            if (agent->radios.empty()) {
                LOG(ERROR) << "agent with mac_addr " << agent_mac_addr_str
                           << " has no radios, un_station with mac_addr " << new_station_mac_add_str
                           << " will NOT be added.";
                return nullptr;
            }
        ...
        if (!add_un_station_dm(agent_mac_addr_str, tlvf::mac_to_string(agent_radio->radio_uid),
                               new_station_mac_add_str)) {
            LOG(ERROR) << "Failed to add unassoc sta to dm";
            return nullptr;
        }
        ...
        // Always store the agent's AL-MAC, never ZERO_MAC (used as "all agents" sentinel).
        new_updated_station->add_agent(agent->al_mac, agent_radio->radio_uid);
                                     ...
                                     sMacAddr const &radio_mac_addr)
{
    ...
    auto remove_un_station_from_dm = [&](std::string agent_mac, std::string station_mac) {
    ...
    if (!all_connected_agents) {
        if (un_station->get_agents().find(agent_mac_addr) != un_station->get_agents().end()) {
            ...
            auto result = remove_un_station_from_dm(tlvf::mac_to_string(agent_mac_addr),
                                                    tlvf::mac_to_string(mac_address));
            if (!result) {
                LOG(ERROR) << "Unassoc sta " << tlvf::mac_to_string(mac_address)
                           << " was not removed from the datamodel!!";
                return false;
            }

            un_station->remove_agent(agent_mac_addr);
            LOG(DEBUG) << "removed monitoring agent " << tlvf::mac_to_string(agent_mac_addr)
                       << " from un_station " << tlvf::mac_to_string(mac_address);

            // Only erase the station entirely when no agents are left monitoring it.
            if (un_station->get_agents().empty()) {
        ...
        bool dm_op_failed = false;
        for (auto &agent : un_station->get_agents()) {
            auto result = remove_un_station_from_dm(tlvf::mac_to_string(agent.first),
                                                    tlvf::mac_to_string(mac_address));
            if (result == false) {
            ...
            un_station->remove_agent(agent.first);
            LOG(DEBUG) << "removed monitoring agent " << tlvf::mac_to_string(agent.first)
                       << " from un_station " << tlvf::mac_to_string(mac_address);
        }
...
bool db::parse_dpp_bootstrap_info(const std::string &dpp_uri, sDppBootstrappingInfo &info,
                                  std::string &error)
{
    error.clear();
    ...
    if ((uri_normalized.size() < suffix.size()) ||
        (uri_normalized.compare(uri_normalized.size() - suffix.size(), suffix.size(), suffix) !=
         0)) {
    ...
    std::string body =
        uri_normalized.substr(prefix.size(), uri_normalized.size() - prefix.size() - suffix.size());
    ...
    bool seen_c = false, seen_m = false, seen_i = false, seen_v = false, seen_h = false,
         seen_k                     = false;
    ...
    for (const auto &field : fields) {
        ...
        case 'C': {
            ...
            for (const auto &pair : pairs) {
                ...
                if ((errno != 0) || (end == parts[0].c_str()) || (*end != '\0') || (op_class < 0) ||
                    (op_class > 255)) {
                ...
                if ((errno != 0) || (end == parts[1].c_str()) || (*end != '\0') || (channel < 0) ||
                    (channel > 255)) {
                ...
                info.operating_class_channel.emplace(static_cast<uint8_t>(op_class),
        ...
        case 'V': {
            ...
            if ((errno != 0) || (end == value.c_str()) || (*end != '\0') || (version < 0) ||
                (version > 255)) {
        ...
        case 'K': {
            ...
            if (out_len < 0) {
                error = "OpenSSL base64 decode failed";
                return false;
            }
...
const db::sDppBootstrappingInfo *
db::get_dpp_bootstrap_info_by_pkhash(const std::array<uint8_t, 32> &pkhash) const
{
