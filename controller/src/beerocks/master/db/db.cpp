...
bool db::dm_clear_sta_stats(const sMacAddr &sta_mac)
{
    ...
    auto station = get_station(sta_mac);
    auto sta_mld = station ? get_sta_mld(station->al_mac, sta_mac) : nullptr;
    if (sta_mld) {
        for (const auto &affiliated_sta_entry : sta_mld->affiliated_stas) {
            ...
            dm_set_sta_link_metrics(station->al_mac, affiliated_sta->mac, affiliated_sta->bssid, 0,
                                    0, 0);
            metrics.bssid = affiliated_sta->bssid;
            dm_set_sta_extended_link_metrics(station->al_mac, affiliated_sta->mac, metrics);
        }
    }
