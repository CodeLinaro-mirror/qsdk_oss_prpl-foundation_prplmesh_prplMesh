...
void dynamic_channel_selection_r2_task::abort_active_scans_in_agent(const sMacAddr agent_mac,
                                                                    sAgentScanStatus &agent_status)
{
    LOG(ERROR) << "aborting all scans for agent " << agent_mac;

    if (!agent_status.single_radio_scans.empty()) {
        // remove all radio_scan_request in-progress from agent queue
        auto scan_it = agent_status.single_radio_scans.begin();
    ...
    if (!agent_status.continuous_radio_scans.empty()) {
        auto scan_it = agent_status.continuous_radio_scans.begin();
        while (scan_it != agent_status.continuous_radio_scans.end()) {
            if (scan_it->second.status != eRadioScanStatus::PENDING) {
                ...
                if (database.get_channel_scan_is_enabled(scan_it->first)) {
                    ...
                    LOG(WARNING) << "aborting scan for radio: " << radio_mac;
                    scan_it = agent_status.continuous_radio_scans.erase(scan_it);
                }
            } else {
                ++scan_it;
            }
...
bool dynamic_channel_selection_r2_task::trigger_pending_scan_request_for_agent(
    const sMacAddr &agent_mac, sAgentScanStatus &agent_status)
{
    // Triggering a scan request for a busy agent will result in the abort
    // of the running scan on that agent. Therefore, triggering of a new scan
    // will only be performed after the current scan is complete (marked by idle agent).
    ...
    if ((!create_channel_scan_request_message(agent_mac, mid, channel_scan_request_tlv)) ||
        ...
        abort_active_scans_in_agent(agent_mac, agent_status);
        return false; //CMDU creation failed - Trigger the next agent
    }
    ...
    for (auto &radio_scan_request : radio_scan_requests_to_trigger) {
        ...
        if (radio_scan_request.second.is_single_scan) {
            ...
            agent_status.continuous_radio_scans[radio_scan_request.first].status =
                eRadioScanStatus::SCAN_IN_PROGRESS;
        }
        ...
        if (!add_radio_to_channel_scan_request_tlv(channel_scan_request_tlv, scan_report_index,
                                                   radio_mac,
                                                   radio_scan_request.second.is_single_scan)) {
            // Failed to add radio to radio_list in channel_scan_request_tlv
            LOG(ERROR) << "add_radio_to_channel_scan_request_tlv() failed for radio " << radio_mac;
            success = false;
            break;
        }
        ...
        if (radio_scan_request.second.is_single_scan) {
            agent_status.single_radio_scans[radio_mac].scan_report_index = scan_report_index;
        } else {
            agent_status.continuous_radio_scans[radio_mac].scan_report_index = scan_report_index;
        }
    }
    if (!success) {
        abort_active_scans_in_agent(agent_mac, agent_status);
    ...
    if (database.is_prplmesh(agent_mac)) {
        auto channel_scan_request_extension_vs_tlv =
            beerocks::message_com::add_vs_tlv<beerocks_message::tlvVsChannelScanRequestExtension>(
                cmdu_tx);
    ...
    if (!success) {
        abort_active_scans_in_agent(agent_mac, agent_status);
        return false; //sending scan request to one of the agents failed - Trigger the next agent
    }

    agent_status.status  = eAgentStatus::BUSY;
    agent_status.timeout = std::chrono::system_clock::now() + CHANNEL_SCAN_REPORT_WAIT_TIME;
...
bool dynamic_channel_selection_r2_task::trigger_pending_scan_requests()
{
    for (auto &agent : m_agents_scan_status_map) {
        ...
        if (!trigger_pending_scan_request_for_agent(agent_mac, agent_status)) {
            LOG(ERROR) << "fail: agent status " << int(agent_status.status);
        } else {
            LOG(DEBUG) << "success : agent status " << int(agent_status.status);
        }
    }
    return true;
}
