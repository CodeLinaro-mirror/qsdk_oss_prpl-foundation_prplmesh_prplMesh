...
void ieee1905_task::set_ieee1905_network_enabled(bool enabled)
{
    ...
    if (!database.ieee1905_network) {
        ...
        m_als.clear();
        status_pending = SingleShotCounter(1, [this]() { set_network_status("Available"); });
...
bool ieee1905_task::start_remote_al_discovery(const sMacAddr &al_mac)
{
    ...
    al.info_pending = SingleShotCounter(2, [this, al_mac]() { complete_remote_al(al_mac); });
    al.topology_response_pending     = SingleShotCounter(1, [this, al_mac]() {
        auto al_it = m_als.find(al_mac);
        if (al_it == m_als.end()) {
            return;
    ...
    al.higher_layer_response_pending = SingleShotCounter(1, [this, al_mac]() {
        auto al_it = m_als.find(al_mac);
        if (al_it == m_als.end()) {
            return;
        }
